...
app = modal.App("eeg-vlm-v60")

ckpt_vol = modal.Volume.from_name("bt-checkpoints-v60", create_if_missing=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
# V6.0: Keep the improved V5.0 encoder path, but replace the weak Phase 2
# chat-style interface with structured denoising over multiple CTC candidates.
#
# What V4.2 proved:
...
#
# Big upgrade:
#   [P2-1] Replace chat-template prompts with structured denoising prompts.
#   [P2-2] Feed top-3 CTC beams with normalized scores, not only one hint string.
#   [P2-3] Use teacher hints corrupted to look like actual CTC outputs
#          (vowel-preserving, consonant-dropping, repeated-char noise).
#   [P2-4] Warm-start Phase 2 directly from V5.0 Phase 1 best checkpoint.
#
...
def run_pipeline(epochs_p1: int = 24, epochs_p2: int = 40):
    ...
    model = EEG_VLM_V60(q_name).to(torch.bfloat16).cuda()

    # ── Resume logic ──────────────────────────────────────────────────────────
    import glob as _glob
    p1_ckpts  = sorted(_glob.glob("/persist/v60_phase1_ep*.pt"))
    p2_ckpts  = sorted(_glob.glob("/persist/v60_phase2_ep*.pt"))
    best_p1   = "/persist/v60_phase1_best.pt"
    final     = "/persist/v60_final.pt"
    source_v50_best = "/v50/v50_phase1_best.pt"
    p1_start_epoch = 0

    if Path(final).exists():
        print("✓ V6.0 already complete.")
        return
    ...
    elif Path(source_v50_best).exists():
        model.load_state_dict(torch.load(source_v50_best, map_location="cuda"))
        print("✓ Loaded V5.0 best Phase 1 checkpoint. Starting V6.0 directly from Phase 2.")
        skip_p1 = True
    else:
    ...
    print(f"\n{'='*60}")
    print(f" V6.0: structured multi-beam denoising from V5.0 Phase 1")
    ...
    if not skip_p2:
        ...
        _train_phase(model, phase=2, epochs=epochs_p2_remaining,
                     ...
                     tokenizer=tokenizer, ckpt_prefix="/persist/v60",
                     mem_bank=mem_bank)

    torch.save(model.state_dict(), "/persist/v60_final.pt")
    ckpt_vol.commit()
    print("\n✓ V6.0 complete. Saved /persist/v60_final.pt")


@app.local_entrypoint()
def main(mode: str = "pipeline", epochs_p1: int = 24, epochs_p2: int = 40):
    if mode == "pipeline":
        print(f"Launching V6.0: Phase1={epochs_p1}ep, Phase2={epochs_p2}ep")
