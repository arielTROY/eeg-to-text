...
app = modal.App("eeg-vlm-v33")

ckpt_vol = modal.Volume.from_name("bt-checkpoints-v33", create_if_missing=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
# V3.3: V3.1b with consonant boost REMOVED from Phase 1
#
...
def run_pipeline(epochs_p1: int = 40, epochs_p2: int = 40):
    ...
    model = EEG_VLM_V33(q_name).to(torch.bfloat16).cuda()

    # ── Resume logic ──────────────────────────────────────────────────────────
    import glob as _glob
    p1_ckpts  = sorted(_glob.glob("/persist/v33_phase1_ep*.pt"))
    p2_ckpts  = sorted(_glob.glob("/persist/v33_phase2_ep*.pt"))
    best_p1   = "/persist/v33_phase1_best.pt"
    final     = "/persist/v33_final.pt"

    if Path(final).exists():
        print("✓ V3.3 already complete.")
        return
    ...
    print(f"\n{'='*60}")
    print(f" V3.3: Plain CTC + V3.1b Phase 2 improvements")
    ...
    if not skip_p1:
        _train_phase(model, phase=1, epochs=epochs_p1,
                     ...
                     tokenizer=tokenizer, ckpt_prefix="/persist/v33")
    ...
    torch.save(model.state_dict(), "/persist/v33_final.pt")
    ckpt_vol.commit()
    print("\n✓ V3.3 complete. Saved /persist/v33_final.pt")


@app.local_entrypoint()
def main(mode: str = "pipeline", epochs_p1: int = 40, epochs_p2: int = 40):
    if mode == "pipeline":
        print(f"Launching V3.3: Phase1={epochs_p1}ep, Phase2={epochs_p2}ep")
