"""
V1200: Stage 2 Relaunch with Relational Distillation on enc_mean
=================================================================
ROOT CAUSE ANALYSIS — Why V1100 collapsed at ep3:
  V1100 had no diversity pressure on eeg_to_t5's output.
  VideoMAE frozen → v_seq is diverse (51.7% from Stage 1).
  But eeg_to_t5 can still collapse: mapping all diverse v_seq to the SAME enc
  minimizes LM loss faster (T5 just memorizes the mode distribution).
  By ep3: div=1.5%, all preds = "He was born in Brooklyn, New York."

V1200 FIX — Relational Distillation on enc_mean:
  enc_mean = eeg_to_t5(v_seq).mean(1)  → (B, 768)
  loss_rel = MSE(eeg_pairwise_cosine_sim, sbert_pairwise_cosine_sim)
  This is non-zero gradient even when enc_mean partially collapses.
  Preserves diversity in eeg_to_t5 output — not just in frozen VideoMAE.

  + Diverse beam search (num_beam_groups=5, diversity_penalty=1.0, num_beams=10)
    Breaks T5 "He was born in Brooklyn, New York." mode fixation.

WHY rel_loss ON enc_mean (not v_mean):
  v_mean is FROZEN → no gradient can flow through it.
  enc_mean = eeg_to_t5(v_seq).mean(1) IS trainable.
  rel_loss forces eeg_to_t5 to preserve the structural diversity of
  the frozen VideoMAE features when projecting them to T5 cross-attention space.
"""
...
app       = modal.App("eeg-vlm-v1200")
ckpt_vol  = modal.Volume.from_name("bt-checkpoints-v1200", create_if_missing=True)
...
W_ALIGN = 0.5   # keeps EEG semantically grounded (InfoNCE)
W_REL   = 0.5   # relational distillation on enc_mean — prevents eeg_to_t5 collapse
...
class EEGV1200(nn.Module):
    """
    V1200: VideoMAE FROZEN (from V1000 ep15, 51.7% diversity).
    eeg_to_t5 RANDOMLY INITIALIZED — fresh mapping from discriminative features → T5.
    + Relational distillation on enc_mean: prevents eeg_to_t5 collapse.
    + Diverse beam search: breaks T5 mode fixation.
    Trainable: eeg_to_t5, sem_proj, txt_proj, ch_adapt, aux heads.
    Frozen: VideoMAE + T5.
    """
    def __init__(self):
        super().__init__()
    ...
    def get_eeg_enc(self, traj):
        ...
        enc_mean = enc.mean(1)                                     # (B,768) — for rel_loss
    ...
    def forward(self, traj, sbert_embs, target_ids, target_mask,
        ...
        # Relational distillation on enc_mean — prevents eeg_to_t5 collapse
        # Force pairwise cosine structure of enc_mean to match SBERT pairwise structure
        loss_rel = torch.tensor(0.0, device=device)
        if B > 1:
            k_norm   = F.normalize(sbert_embs.to(device, dtype), dim=-1)  # (B,768)
            e_norm   = F.normalize(enc_mean.to(dtype), dim=-1)             # (B,768)
            eeg_sim  = e_norm @ e_norm.T                                    # (B,B)
            txt_sim  = (k_norm @ k_norm.T).detach()                        # (B,B)
...
def train(model, train_ds, val_ds, tokenizer, device, epochs=35):
    ...
    for epoch in range(1, epochs + 1):
        ...
        if score > best_score or wer < best_wer:
            ...
            ckpt_path  = ckpt_dir / f"v1200_ep{epoch}_div{div:.3f}_cr{cr:.3f}_wer{wer:.3f}.pt"
            torch.save({
...
def run_training(epochs: int = 35):
    ...
    print(f"Strategy: VideoMAE FROZEN (Stage 1 features preserved) + fresh eeg_to_t5 + rel_loss on enc_mean")
