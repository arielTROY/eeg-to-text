...
app = modal.App("eeg-vlm-v301-acoustic-ctc")

ckpt_vol = modal.Volume.from_name("bt-checkpoints-v301", create_if_missing=True)
teacher_vol = modal.Volume.from_name("bt-checkpoints-v300", create_if_missing=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
# V301: EEG-video to Whisper latent bridge with grounded wordpiece CTC readout.
#
# What changes:
...
#   [4] decode with grounded wordpiece CTC over aligned bridge states
#
# The goal is to move the target from text to speech-like acoustic/phonetic
# latents, while keeping the readout tied to EEG evidence instead of a
# free-running text decoder.
# ═══════════════════════════════════════════════════════════════════════════════
...
WHISPER_CACHE_DIR = Path("/v300/whisper_teacher_cache")
...
def build_whisper_teacher_cache(texts, device="cuda"):
    ...
    for text in unique_texts:
        ...
        if not pt_path.exists():
            missing.append(text)
    if not missing:
        print(f"[WhisperTeacher] cache ready: {len(unique_texts)} / {len(unique_texts)}")
        return
    ...
    for idx, text in enumerate(missing, 1):
        ...
        built += 1
        if built % 100 == 0:
            print(f"[WhisperTeacher] built {built}/{len(missing)}")
            teacher_vol.commit()
    del model
    teacher_vol.commit()
...
class EEGWhisperBridge(nn.Module):
    def __init__(self, ngram_dim):
        ...
        self.wp_ctc_head = nn.Linear(384, WP_VOCAB_SIZE)
    ...
    def forward(self, traj, wp_ids, wp_lens, char_ids, char_lens, phone_ids, phone_lens,
                cons_targets, ngram_targets, boundary_targets, teacher_states, teacher_lens):
        B = traj.shape[0]
        device = traj.device
        ...
        wp_logits = self.wp_ctc_head(resampled_bridge).transpose(0, 1)
        ...
        wp_input_lens = teacher_lens
        loss_wp_ctc = self.wp_ctc_loss_fn(
            F.log_softmax(wp_logits, dim=-1), wp_ids, wp_input_lens, wp_lens
        )
    ...
    @torch.no_grad()
    def decode_candidates(self, traj, nbest=5):
        ...
        wp_logits = self.wp_ctc_head(resampled_bridge).transpose(0, 1)
        char_logits = self.ctc_head(memory).transpose(0, 1)
        phone_logits = self.phone_ctc_head(memory).transpose(0, 1)
        cons_probs = torch.sigmoid(self.cons_head(memory.mean(dim=1))).float().cpu().numpy()
        ngram_probs = torch.sigmoid(self.ngram_head(memory.mean(dim=1))).float().cpu().numpy()
        boundary_probs = torch.sigmoid(self.boundary_head(memory).squeeze(-1)).float().cpu().numpy()
        phone_probs = F.softmax(phone_logits.transpose(0, 1), dim=-1).mean(dim=1).float().cpu().numpy()

        char_beams_all = beam_ctc_decode_nbest(
            char_logits, beam_width=12, nbest=nbest, char_lm=CHAR_LM, lm_weight=0.35, diversity_weight=0.16
        )
        wp_beams_all = beam_ctc_decode_wp_nbest(
            wp_logits, beam_width=12, nbest=nbest, blank_penalty=0.10, diversity_weight=0.18
        )
        merged_all = []
        for i in range(len(char_beams_all)):
            merged = merge_char_wp_candidates(
                char_beams_all[i], wp_beams_all[i], ngram_probs[i], cons_probs[i], boundary_probs[i], phone_probs[i], nbest=nbest
            )
            merged_all.append(merged)
        phone_preds = greedy_phone_ctc_decode(phone_logits)
        return merged_all, char_beams_all, wp_beams_all, phone_preds

    @torch.no_grad()
    def generate(self, traj):
        merged_all, _, _, _ = self.decode_candidates(traj, nbest=1)
        return [cands[0][0] if cands else "" for cands in merged_all]
...
class EEGDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return eeg, s['text'], wp_ids, char_ids, phone_ids, cons_target, ngram_target, boundary_target, teacher


def collate_fn(batch):
    eegs, texts, wp_ids_list, char_ids_list, phone_ids_list, cons_targets, ngram_targets, boundary_targets, teacher_list = zip(*batch)
    ...
    return (
        eegs, list(texts), wp_pad, wp_lens, char_pad, char_lens, phone_pad, phone_lens,
        cons_tensor, ngram_tensor, boundary_tensor,
        teacher_pad, teacher_lens,
    )
...
def make_phase1_optimizer(model, steps_per_epoch, epochs, freeze_encoder):
    ...
    add_group(model.wp_ctc_head.parameters(), 1.4e-4 if freeze_encoder else 1.2e-4, "wp_ctc")
...
def _validate_phase1(model, val_loader, epoch, ckpt_prefix, best_phono_top1, best_oracle_wer):
    ...
    model.eval()
    cer_scores, wer_scores, oracle_wer_scores, per_scores = [], [], [], []
    ...
    with torch.no_grad():
        ...
        for batch in val_loader:
            ...
            merged_all, char_beams_all, wp_beams_all, phone_preds = model.decode_candidates(
                eeg_v.to(device).to(torch.bfloat16), nbest=5
            )
            for b_idx, (merged, ref) in enumerate(zip(merged_all, texts_v)):
                ref_lower = ref.lower()
                top = merged[0][0] if merged else ""
                oracle = min(compute_wer(text, ref_lower) for text, _ in merged) if merged else 2.0
                ...
                if len(ref.split()) > 3 or shown < 2:
                    print(f"  REF: '{ref[:100]}'")
                    print(f"  TOP1: '{top[:100]}'  CER={cer_scores[-1]:.2f} WER={wer_scores[-1]:.2f} O-WER={oracle:.2f}")
                    print(f"  WPC: '{wp_beams_all[b_idx][0][0][:100] if wp_beams_all[b_idx] else ''}'")
                    print(f"  CTC: '{char_beams_all[b_idx][0][0][:100] if char_beams_all[b_idx] else ''}'")
...
def _train_phase1_stage(model, epochs, train_loader, val_loader, ckpt_prefix,
    ...
    for local_epoch in range(1, epochs + 1):
        ...
        model.train()
        tot_wp = tot_align = tot_len = tot_ctc = tot_phone = tot_cons = tot_ngram = tot_bnd = tot_div = 0.0
        optimizer.zero_grad()

        for step, batch in enumerate(train_loader):
            eeg, _, wp_ids, wp_lens, char_ids, char_lens, phone_ids, phone_lens, cons_targets, ngram_targets, boundary_targets, teacher_states, teacher_lens = batch
            ...
            with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                out = model(
                    eeg, wp_ids, wp_lens, char_ids, char_lens, phone_ids, phone_lens,
                    cons_targets, ngram_targets, boundary_targets, teacher_states, teacher_lens
                )
                loss = (
                    2.6 * out["loss_wp_ctc"] +
        ...
        print(
            f"Epoch {epoch:3d} | WP:{tot_wp/n:.3f} ALN:{tot_align/n:.3f} LEN:{tot_len/n:.3f} CTC:{tot_ctc/n:.3f} "
        ...
        if (tot_wp / n) < best_train_ctc:
            ...
            print(f"  ✓ New best train WP {best_train_ctc:.3f}")
...
@app.function(image=image, gpu="H100", timeout=86400,
              volumes={"/data": data_vol, "/persist": ckpt_vol, "/v300": teacher_vol},
...
def run_pipeline(epochs_p1: int = 2, use_innerspeech: bool = False):
    ...
    global NGRAM_LIST, NGRAM_TO_ID, SP_MODEL, WP_VOCAB_SIZE, CHAR_LM
    ...
    CHAR_LM = CharNGramLM(train_texts_for_vocab, order=3)
    SP_MODEL = build_sentencepiece_model(train_texts_for_vocab, model_prefix="/v300/v300_wordpiece", vocab_size=192)
    WP_VOCAB_SIZE = SP_MODEL.get_piece_size() + 1
    DEC_PAD_ID = 0
    DEC_BOS_ID = WP_VOCAB_SIZE
    DEC_EOS_ID = WP_VOCAB_SIZE + 1
    DEC_VOCAB_SIZE = WP_VOCAB_SIZE + 2
    print(f"[V301] ngram vocab size: {len(NGRAM_LIST)}")
    print(f"[V301] wordpiece vocab size: {WP_VOCAB_SIZE - 1}")
    ...
    print(f"\n{'='*60}")
    print(" V301: EEG-video to Whisper latent bridge + grounded wordpiece CTC")
    print(" Exact topography + hierarchical VideoMAE + aligned acoustic-state decode")
    print(" FRESH START")
    print(f"{'='*60}\n")
    ...
    best_p1 = "/persist/v301_phase1_best.pt"
    if Path(best_p1).exists():
        model.load_state_dict(torch.load(best_p1, map_location="cuda"))

    torch.save(model.state_dict(), "/persist/v301_final.pt")
    ckpt_vol.commit()
    print("\n✓ V301 complete. Saved /persist/v301_final.pt")


@app.local_entrypoint()
def main(mode: str = "pipeline", epochs_p1: int = 2, use_innerspeech: bool = False):
    if mode == "pipeline":
        print(f"Launching V301: Phase1={epochs_p1}ep inner={use_innerspeech}")
