...
app = modal.App("eeg-vlm-v14-ctc")

ckpt_vol = modal.Volume.from_name("bt-checkpoints-v14", create_if_missing=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
# V14: V13 plus local chunk supervision and LM-guided diverse decode.
#
# What failed:
...
#
# This branch keeps the V13 encoder path intact and adds:
#   [1] sentence-level grapheme chunk supervision (top bigrams/trigrams)
#   [2] LM-guided diverse CTC beam search
# The intent is to convert "some consonants are present" into actual part-word
# structure without destabilizing the alignment regime that finally works.
# ═══════════════════════════════════════════════════════════════════════════════
...
class EEG_CTC_V14(nn.Module):
    ...
    def forward(self, traj, char_ids, char_lens, class_ids, class_lens, cons_targets, ngram_targets):
        B = traj.shape[0]
        device = traj.device
    ...
    def decode_ctc(self, traj, beam_width=16, nbest=5):
        ...
        v_seq = self.encode(traj)
        ctc_logits = self.ctc_head(v_seq).transpose(0, 1)
        return beam_ctc_decode_nbest(
            ctc_logits, beam_width=beam_width, nbest=nbest, char_lm=self.char_lm)
...
class EEGDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return eeg, s['text'], char_ids, class_ids, cons_target, ngram_target


def collate_fn(batch):
    eegs, texts, char_ids_list, class_ids_list, cons_targets, ngram_targets = zip(*batch)
    ...
    return eegs, list(texts), char_pad, char_lens, class_pad, class_lens, cons_tensor, ngram_tensor
...
def _train_phase1_stage(model, epochs, train_loader, val_loader, ckpt_prefix,
    ...
    for local_epoch in range(1, epochs + 1):
        ...
        model.train()
        tot_ctc = tot_class = tot_cons = tot_ngram = tot_div = 0.0
        optimizer.zero_grad()

        for step, batch in enumerate(train_loader):
            eeg, _, c_ids, c_lens, class_ids, class_lens, cons_targets, ngram_targets = batch
            ...
            with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                out = model(eeg, c_ids, c_lens, class_ids, class_lens, cons_targets, ngram_targets)
                loss = (
                    10.0 * out["loss_ctc"] +
                    0.8 * out["loss_class"] +
                    0.35 * out["loss_cons"] +
        ...
        print(
            ...
            f"CONS:{tot_cons/n:.3f} NGRAM:{tot_ngram/n:.3f} Div:{tot_div/n:.4f} | LR0:{lr:.2e}"
        )
...
@app.function(image=image, gpu="H100", timeout=86400,
              volumes={"/data": data_vol, "/persist": ckpt_vol, "/v13": v13_vol, "/v20": v20_vol, "/v50": v50_vol, "/v12": v12_vol},
...
def run_pipeline(epochs_p1: int = 24):
    ...
    print(f"[V14] ngram vocab size: {len(NGRAM_LIST)}")

    model = EEG_CTC_V14(ngram_dim=len(NGRAM_LIST)).to(torch.bfloat16).cuda()
    model.char_lm = CharNGramLM(train_texts_for_vocab, order=3)

    # ── Resume logic ──────────────────────────────────────────────────────────
    import glob as _glob
    p1_ckpts = sorted(_glob.glob("/persist/v14_phase1_ep*.pt"))
    best_p1 = "/persist/v14_phase1_best.pt"
    final = "/persist/v14_final.pt"
    p1_start_epoch = 0
    best_val_cer = float("inf")
    best_train_ctc = float("inf")

    if Path(final).exists():
        print("✓ V14 already complete.")
        return
    ...
    print(f"\n{'='*60}")
    print(" V14: local chunk supervision + LM-guided decode")
    print(" V13 warm-start + chunk inventory + diverse LM beam")
    ...
    torch.save(model.state_dict(), "/persist/v14_final.pt")
    ckpt_vol.commit()
    print("\n✓ V14 complete. Saved /persist/v14_final.pt")


@app.local_entrypoint()
def main(mode: str = "pipeline", epochs_p1: int = 24):
    if mode == "pipeline":
        print(f"Launching V14: Phase1={epochs_p1}ep")
