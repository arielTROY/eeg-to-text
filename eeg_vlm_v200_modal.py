...
app = modal.App("eeg-vlm-v200-hierarchical-seq2seq")

ckpt_vol = modal.Volume.from_name("bt-checkpoints-v200", create_if_missing=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
# V200: hierarchical clip-video seq2seq.
#
# What changes:
#   [1] no CTC as the main objective
#   [2] encode EEG-video as a sequence of short clips, not one monolithic clip
#   [3] decode text autoregressively from clip tokens
#   [4] keep only local phone/boundary auxiliaries on the encoder side
#
# The goal is to stop forcing framewise spelling and instead let the model
# compose text from hierarchical spatiotemporal chunks.
# ═══════════════════════════════════════════════════════════════════════════════
...
class EEGSeq2Seq_V200(nn.Module):
    ...
    def decode_teacher(self, memory, dec_in):
        B, L = dec_in.shape
    ...
    def forward(self, traj, phone_ids, phone_lens, cons_targets, ngram_targets, boundary_targets, dec_in, dec_out):
        B = traj.shape[0]
        device = traj.device
        memory = self.encode(traj)
        dec_logits = self.decode_teacher(memory, dec_in)
    ...
    @torch.no_grad()
    def generate(self, traj, max_len=48):
        traj = traj.to(next(self.parameters()).dtype)
        memory = self.encode(traj)
        B = traj.shape[0]
        generated = torch.full((B, 1), DEC_BOS_ID, dtype=torch.long, device=traj.device)
        finished = torch.zeros(B, dtype=torch.bool, device=traj.device)
        for _ in range(max_len):
            logits = self.decode_teacher(memory, generated)
            next_tok = logits[:, -1, :].argmax(dim=-1, keepdim=True)
            generated = torch.cat([generated, next_tok], dim=1)
            finished |= (next_tok.squeeze(1) == DEC_EOS_ID)
            if bool(finished.all()):
                break
        return [decode_decoder_ids(seq.tolist()) for seq in generated[:, 1:]]
...
def _train_phase1_stage(model, epochs, train_loader, val_loader, ckpt_prefix,
    ...
    for local_epoch in range(1, epochs + 1):
        ...
        model.train()
        tot_seq = tot_phone = tot_cons = tot_ngram = tot_bnd = tot_div = 0.0
        optimizer.zero_grad()

        for step, batch in enumerate(train_loader):
            eeg, _, _, _, _, _, phone_ids, phone_lens, _, _, cons_targets, ngram_targets, boundary_targets, dec_in, dec_out, dec_lens = batch
            ...
            with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                out = model(
                    eeg, phone_ids, phone_lens, cons_targets, ngram_targets, boundary_targets, dec_in, dec_out
                )
                loss = (
                    4.0 * out["loss_seq"] +
        ...
        print(
            f"Epoch {epoch:3d} | SEQ:{tot_seq/n:.3f} PCTC:{tot_phone/n:.3f} CONS:{tot_cons/n:.3f} NGRAM:{tot_ngram/n:.3f} "
...
def run_pipeline(epochs_p1: int = 3, use_innerspeech: bool = False):
    ...
    SP_MODEL = build_sentencepiece_model(train_texts_for_vocab, model_prefix="/persist/v200_wordpiece", vocab_size=192)
    WP_VOCAB_SIZE = SP_MODEL.get_piece_size() + 1
    DEC_PAD_ID = 0
    DEC_BOS_ID = WP_VOCAB_SIZE
    DEC_EOS_ID = WP_VOCAB_SIZE + 1
    DEC_VOCAB_SIZE = WP_VOCAB_SIZE + 2
    print(f"[V200] ngram vocab size: {len(NGRAM_LIST)}")
    print(f"[V200] wordpiece vocab size: {WP_VOCAB_SIZE - 1}")
    ...
    p1_ckpts = sorted(_glob.glob("/persist/v200_phase1_ep*.pt"))
    best_p1 = "/persist/v200_phase1_best.pt"
    final = "/persist/v200_final.pt"
    p1_start_epoch = 0
    best_phono_top1 = -1.0
    best_oracle_wer = float("inf")
    ...
    print(f"\n{'='*60}")
    print(" V200: hierarchical clip-video seq2seq")
    print(" Exact topography + clip VideoMAE + direct wordpiece decoding")
    ...
    torch.save(model.state_dict(), "/persist/v200_final.pt")
    ckpt_vol.commit()
    print("\n✓ V200 complete. Saved /persist/v200_final.pt")


@app.local_entrypoint()
def main(mode: str = "pipeline", epochs_p1: int = 3, use_innerspeech: bool = False):
    if mode == "pipeline":
        print(f"Launching V200: Phase1={epochs_p1}ep inner={use_innerspeech}")
