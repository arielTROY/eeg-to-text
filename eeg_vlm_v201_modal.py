...
app = modal.App("eeg-vlm-v201-grounded-seq2seq")

ckpt_vol = modal.Volume.from_name("bt-checkpoints-v201", create_if_missing=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
# V201: grounded hierarchical clip-video seq2seq.
#
# What changes:
#   [1] keep the clip-video seq2seq decoder from V200
#   [2] add a wordpiece-CTC auxiliary on the encoder memory
#   [3] use the wordpiece-CTC prior during generation to keep decoding grounded
#   [4] keep local phone/boundary auxiliaries
#
# The goal is to keep V200's more text-like outputs without letting the decoder
# ignore the EEG evidence and fall into generic repetition.
# ═══════════════════════════════════════════════════════════════════════════════
...
class EEGSeq2Seq_V200(nn.Module):
    def __init__(self, ngram_dim):
        ...
        self.decoder_tok = nn.Embedding(DEC_VOCAB_SIZE, 768)
    ...
    def decode_teacher(self, memory, dec_in, prior_bias=None, prior_scale=0.0):
        B, L = dec_in.shape
        if L > self.decoder_pos.num_embeddings:
            dec_in = dec_in[:, : self.decoder_pos.num_embeddings]
            L = dec_in.shape[1]
        pos = torch.arange(L, device=dec_in.device).unsqueeze(0)
        tgt = self.decoder_tok(dec_in) + self.decoder_pos(pos)
        causal_mask = torch.triu(
            torch.full((L, L), float("-inf"), device=dec_in.device),
            diagonal=1,
        )
    ...
    def forward(self, traj, wp_ids, wp_lens, phone_ids, phone_lens, cons_targets, ngram_targets, boundary_targets, dec_in, dec_out):
        ...
        dec_logits = self.decode_teacher(memory, dec_in, prior_bias=prior_bias, prior_scale=TRAIN_GROUND_SCALE)
    ...
    def generate(self, traj, max_len=48, beam_width=4):
        ...
        for b in range(B):
            mem_b = memory[b:b+1]
            prior_b = prior_bias[b:b+1]
            beams = [([DEC_BOS_ID], 0.0, False)]
            for _ in range(max_len):
                cand = []
                for seq, score, done in beams:
                    if done:
                        cand.append((seq, score, done))
                        continue
                    dec_in = torch.tensor(seq, dtype=torch.long, device=traj.device).unsqueeze(0)
                    logits = self.decode_teacher(mem_b, dec_in, prior_bias=prior_b, prior_scale=INFER_GROUND_SCALE)
                    step_log_probs = F.log_softmax(logits[:, -1, :], dim=-1).squeeze(0)
                    step_log_probs = self._penalize_next_token(step_log_probs, seq)
                    topk = torch.topk(step_log_probs, beam_width).indices.tolist()
                    for tok in topk:
                        tok_score = step_log_probs[tok].item()
                        new_seq = seq + [tok]
                        cand.append((new_seq, score + tok_score, tok == DEC_EOS_ID))
                beams = sorted(
                    cand,
                    key=lambda x: x[1] / max(1, len([t for t in x[0] if t not in (DEC_BOS_ID, DEC_EOS_ID)])) ** 0.7,
                    reverse=True,
                )[:beam_width]
            ...
            outputs.append(decode_decoder_ids(best[0][1:]))
        return outputs
...
def load_v200_phase1_warmstart(model, ckpt_path="/v200/v200_phase1_best.pt"):
    ...
    prefixes = (
        ...
        "decoder_tok.", "decoder_pos.", "decoder.", "decoder_norm.", "decoder_out.",
...
def set_phase1_trainable(model, freeze_encoder):
    ...
    for p in model.temporal_encoder.parameters():
        p.requires_grad = True
    for p in model.decoder_tok.parameters():
        p.requires_grad = True
...
def make_phase1_optimizer(model, steps_per_epoch, epochs, freeze_encoder):
    ...
    add_group(model.decoder_tok.parameters(), 8e-5, "dec_tok")
...
@app.function(image=image, gpu="H100", timeout=86400,
              volumes={"/data": data_vol, "/persist": ckpt_vol, "/v200": v200_vol},
...
def run_pipeline(epochs_p1: int = 2, use_innerspeech: bool = False):
    ...
    sp_prefix = "/v200/v200_wordpiece" if Path("/v200/v200_wordpiece.model").exists() else "/persist/v201_wordpiece"
    ...
    print(f"[V201] ngram vocab size: {len(NGRAM_LIST)}")
    print(f"[V201] wordpiece vocab size: {WP_VOCAB_SIZE - 1}")
    ...
    p1_ckpts = sorted(_glob.glob("/persist/v201_phase1_ep*.pt"))
    best_p1 = "/persist/v201_phase1_best.pt"
    final = "/persist/v201_final.pt"
    p1_start_epoch = 0
    best_phono_top1 = -1.0
    best_oracle_wer = float("inf")
    ...
    print(f"\n{'='*60}")
    print(" V201: grounded hierarchical clip-video seq2seq")
    print(" Exact topography + clip VideoMAE + grounded wordpiece decoding")
    ...
    torch.save(model.state_dict(), "/persist/v201_final.pt")
    ckpt_vol.commit()
    print("\n✓ V201 complete. Saved /persist/v201_final.pt")


@app.local_entrypoint()
def main(mode: str = "pipeline", epochs_p1: int = 2, use_innerspeech: bool = False):
    if mode == "pipeline":
        print(f"Launching V201: Phase1={epochs_p1}ep inner={use_innerspeech}")
