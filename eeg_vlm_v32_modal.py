...
app = modal.App("eeg-vlm-v32")

ckpt_vol = modal.Volume.from_name("bt-checkpoints-v32", create_if_missing=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
# V3.2: V3.1b with consonant boost REMOVED from Phase 1
#
...
#   [B1] Theta band (4-8Hz) replaces V2.0 gamma — language-relevant band
...
def normalize_eeg(eeg, target_ch=64, target_t=1024):
    ...
    theta = band_env(4,   8,  eeg)   # language-relevant theta band
    alpha = band_env(8,  13,  eeg)
    beta  = band_env(13, 30,  eeg)

    combined = np.concatenate([eeg.T, theta.T, alpha.T, beta.T], axis=1)  # (1024, 256)
...
def run_pipeline(epochs_p1: int = 40, epochs_p2: int = 40):
    ...
    model = EEG_VLM_V32(q_name).to(torch.bfloat16).cuda()

    # ── Resume logic ──────────────────────────────────────────────────────────
    import glob as _glob
    p1_ckpts  = sorted(_glob.glob("/persist/v32_phase1_ep*.pt"))
    p2_ckpts  = sorted(_glob.glob("/persist/v32_phase2_ep*.pt"))
    best_p1   = "/persist/v32_phase1_best.pt"
    final     = "/persist/v32_final.pt"

    if Path(final).exists():
        print("✓ V3.2 already complete.")
        return
    ...
    print(f"\n{'='*60}")
    print(f" V3.2: Plain CTC + V3.1b Phase 2 improvements")
    ...
    if not skip_p1:
        _train_phase(model, phase=1, epochs=epochs_p1,
                     ...
                     tokenizer=tokenizer, ckpt_prefix="/persist/v32")
    ...
    torch.save(model.state_dict(), "/persist/v32_final.pt")
    ckpt_vol.commit()
    print("\n✓ V3.2 complete. Saved /persist/v32_final.pt")


@app.local_entrypoint()
def main(mode: str = "pipeline", epochs_p1: int = 40, epochs_p2: int = 40):
    if mode == "pipeline":
        print(f"Launching V3.2: Phase1={epochs_p1}ep, Phase2={epochs_p2}ep")
