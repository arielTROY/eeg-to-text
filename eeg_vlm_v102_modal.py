...
app = modal.App("eeg-vlm-v102-residual-temporal")

ckpt_vol = modal.Volume.from_name("bt-checkpoints-v102", create_if_missing=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
# V102: constrained temporal residual branch.
#
# Keep:
...
#
# Add:
#   [1] a small temporal residual branch over the raw EEG-band timeline
#   [2] frozen V100 base with low-amplitude residual fusion
#   [3] logit stability losses so the temporal path can only correct, not replace
#
# The goal is to keep V100's good beam contents and recover missing local phone
# and consonant timing without another collapse to punctuation.
# ═══════════════════════════════════════════════════════════════════════════════
...
class ResidualTemporalAdapter(nn.Module):
    def __init__(self, in_dim=NUM_EEG_BANDS * 64, hidden_dim=192, out_dim=768):
        super().__init__()
        self.in_proj = nn.Linear(in_dim, hidden_dim)
        self.dw_conv = nn.Conv1d(hidden_dim, hidden_dim, kernel_size=9, padding=4, groups=hidden_dim)
        self.pw_conv = nn.Conv1d(hidden_dim, hidden_dim, kernel_size=1)
    ...
    def forward(self, traj):
        x = self.in_proj(traj)
        x = x.transpose(1, 2)
        x = x + self.pw_conv(F.gelu(self.dw_conv(x)))
...
class EEG_CTC_V102(nn.Module):
    def __init__(self, ngram_dim, wp_vocab_size, phono_dim):
        ...
        self.temporal_adapter = ResidualTemporalAdapter(in_dim=NUM_EEG_BANDS * 64, hidden_dim=192, out_dim=768)
        self.fuse_gate = nn.Parameter(torch.tensor(-3.0))
    ...
    def encode(self, traj):
        ...
        base_seq = v_out.reshape(traj.shape[0], 256, 16, 768).mean(dim=2)
        resid = self.temporal_adapter(traj)
        gate = torch.sigmoid(self.fuse_gate).to(dtype=base_seq.dtype)
        fused = self.fuse_ln(base_seq + gate * resid)
        return base_seq, fused
    ...
    def forward(self, traj, char_ids, char_lens, wp_ids, wp_lens, phone_ids, phone_lens,
        ...
        base_seq, v_seq = self.encode(traj)
        ...
        with torch.no_grad():
            base_ctc_logits = self.ctc_head(base_seq.detach()).transpose(0, 1)
            base_wp_logits = self.wp_ctc_head(base_seq.detach()).transpose(0, 1)
        loss_stability_char = F.kl_div(
            F.log_softmax(ctc_logits, dim=-1),
            F.softmax(base_ctc_logits, dim=-1),
            reduction="batchmean",
        )
        loss_stability_wp = F.kl_div(
            F.log_softmax(wp_logits, dim=-1),
            F.softmax(base_wp_logits, dim=-1),
            reduction="batchmean",
        )
        ...
        return {
            ...
            "loss_stability_char": loss_stability_char,
            "loss_stability_wp": loss_stability_wp,
            "loss_div": loss_div,
        }
...
def load_v100_phase1_warmstart(model, ckpt_path="/v100/v100_phase1_best.pt"):
    ...
    n_copied = n_skipped = 0
    for k, v in src_sd.items():
        if k.startswith("temporal_adapter.") or k.startswith("fuse_gate") or k.startswith("fuse_ln."):
            continue
        if k in dst_sd and dst_sd[k].shape == v.shape:
            dst_sd[k] = v.clone()
            n_copied += 1
...
def set_phase1_trainable(model, freeze_encoder):
    ...
    for p in model.temporal_adapter.parameters():
        p.requires_grad = True
...
def make_phase1_optimizer(model, steps_per_epoch, epochs, freeze_encoder):
    ...
    add_group(model.temporal_adapter.parameters(), 2.5e-4, "temporal")
    add_group([model.fuse_gate], 1.0e-4, "fuse_gate")
    add_group(model.fuse_ln.parameters(), 8.0e-5, "fuse_ln")
...
def _validate_phase1(model, val_loader, epoch, ckpt_prefix, best_phono_top1, best_oracle_wer):
    ...
    with torch.no_grad():
        ...
        for batch in val_loader:
            ...
            _, v_seq = model.encode(eeg_v.to(device).to(torch.bfloat16))
...
def _train_phase1_stage(model, epochs, train_loader, val_loader, ckpt_prefix,
    ...
    for local_epoch in range(1, epochs + 1):
        ...
        model.train()
        tot_char = tot_wp = tot_phone = tot_class = tot_cons = tot_phono = tot_ngram = tot_bnd = tot_div = tot_stab = 0.0
        optimizer.zero_grad()
        ...
        for step, batch in enumerate(train_loader):
            ...
            with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                out = model(
                    ...
                    phono_targets=phono_targets, neg_phono_bank=None
                )
                loss = (
                    5.5 * out["loss_wp_ctc"] +
                    2.5 * out["loss_char_ctc"] +
                    2.0 * out["loss_phone_ctc"] +
                    0.45 * out["loss_class"] +
                    0.25 * out["loss_cons"] +
                    0.20 * out["loss_ngram"] +
                    0.45 * out["loss_boundary"] +
                    1.20 * out["loss_stability_wp"] +
                    0.90 * out["loss_stability_char"] +
                    0.05 * out["loss_div"]
                ) / ACCUM_STEPS
            ...
            tot_stab += (out["loss_stability_wp"].item() + out["loss_stability_char"].item()) / 2.0
        ...
        print(
            ...
            f"BND:{tot_bnd/n:.3f} STAB:{tot_stab/n:.3f} Div:{tot_div/n:.4f} | LR0:{lr:.2e}"
        )
...
def run_pipeline(epochs_p1: int = 3, use_innerspeech: bool = False):
    ...
    SP_MODEL = build_sentencepiece_model(train_texts_for_vocab, model_prefix="/v100/v100_wordpiece", vocab_size=192)
    WP_VOCAB_SIZE = SP_MODEL.get_piece_size() + 1
    print(f"[V102] ngram vocab size: {len(NGRAM_LIST)}")
    print(f"[V102] wordpiece vocab size: {WP_VOCAB_SIZE - 1}")
    ...
    p1_ckpts = sorted(_glob.glob("/persist/v102_phase1_ep*.pt"))
    best_p1 = "/persist/v102_phase1_best.pt"
    final = "/persist/v102_final.pt"
    p1_start_epoch = 0
    best_phono_top1 = -1.0
    best_oracle_wer = float("inf")
    ...
    print(f"\n{'='*60}")
    print(" V102: V100 + constrained temporal residual")
    print(" Frozen V100 base + low-amplitude temporal correction branch")
    ...
    torch.save(model.state_dict(), "/persist/v102_final.pt")
    ckpt_vol.commit()
    print("\n✓ V102 complete. Saved /persist/v102_final.pt")


@app.local_entrypoint()
def main(mode: str = "pipeline", epochs_p1: int = 3, use_innerspeech: bool = False):
    if mode == "pipeline":
        print(f"Launching V102: Phase1={epochs_p1}ep inner={use_innerspeech}")
