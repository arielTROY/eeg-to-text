...
app = modal.App("eeg-vlm-v15-ctc")

ckpt_vol = modal.Volume.from_name("bt-checkpoints-v15", create_if_missing=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
# V15: V14 plus lexical evidence heads and evidence-aware reranking.
#
# What failed:
...
#   [1] a sentence-level word inventory head
#   [2] a word-count head
#   [3] reranking that rewards words/chunks/consonants supported by EEG heads
#       and penalizes unsupported filler words
# The intent is to keep the new diversity from V14 while forcing candidates to
# stay closer to the original sentence.
# ═══════════════════════════════════════════════════════════════════════════════
...
def _extract_candidate_features(text):
    ...
    return words, ngrams, consonants


def rerank_candidates_with_evidence(candidates, word_probs, ngram_probs, cons_probs, count_pred):
    from collections import Counter

    reranked = []
    count_pred = float(count_pred)
    for text, base_score in candidates:
        words, ngrams, consonants = _extract_candidate_features(text)
...
class EEG_CTC_V15(nn.Module):
    def __init__(self, ngram_dim, word_dim):
        super().__init__()
    ...
    def forward(self, traj, char_ids, char_lens, class_ids, class_lens,
                cons_targets, ngram_targets, word_targets, word_counts):
        B = traj.shape[0]
        device = traj.device
    ...
    def decode_ctc(self, traj, beam_width=16, nbest=5):
        ...
        reranked_groups = []
        for beams, wp, ngp, cp, cnt in zip(beam_groups, word_probs, ngram_probs, cons_probs, count_pred):
            reranked = rerank_candidates_with_evidence(beams, wp, ngp, cp, cnt)[:nbest]
            if not reranked:
...
class EEGDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return eeg, s['text'], char_ids, class_ids, cons_target, ngram_target, word_target, word_count


def collate_fn(batch):
    eegs, texts, char_ids_list, class_ids_list, cons_targets, ngram_targets, word_targets, word_counts = zip(*batch)
    ...
    return eegs, list(texts), char_pad, char_lens, class_pad, class_lens, cons_tensor, ngram_tensor, word_tensor, count_tensor
...
def make_phase1_optimizer(model, steps_per_epoch, epochs, freeze_encoder):
    ...
    add_group(model.word_head.parameters(), 8e-5 if freeze_encoder else 1e-4, "word")
    add_group(model.count_head.parameters(), 5e-5 if freeze_encoder else 7e-5, "count")
    if not freeze_encoder:
...
def _validate_phase1(model, val_loader, epoch, ckpt_prefix, best_val_cer):
    ...
    with torch.no_grad():
        ...
        for batch in val_loader:
            eeg_v, texts_v, _, _, _, _, _, _, _, _ = batch
...
def _train_phase1_stage(model, epochs, train_loader, val_loader, ckpt_prefix,
    ...
    for local_epoch in range(1, epochs + 1):
        ...
        model.train()
        tot_ctc = tot_class = tot_cons = tot_ngram = tot_word = tot_count = tot_div = 0.0
        optimizer.zero_grad()

        for step, batch in enumerate(train_loader):
            eeg, _, c_ids, c_lens, class_ids, class_lens, cons_targets, ngram_targets, word_targets, word_counts = batch
...
def run_pipeline(epochs_p1: int = 24):
    ...
    global NGRAM_LIST, NGRAM_TO_ID, WORD_LIST, WORD_TO_ID
    ...
    print(f"[V15] ngram vocab size: {len(NGRAM_LIST)}")
    print(f"[V15] word vocab size: {len(WORD_LIST)}")

    model = EEG_CTC_V15(ngram_dim=len(NGRAM_LIST), word_dim=len(WORD_LIST)).to(torch.bfloat16).cuda()
    model.char_lm = CharNGramLM(train_texts_for_vocab, order=4)

    # ── Resume logic ──────────────────────────────────────────────────────────
    import glob as _glob
    p1_ckpts = sorted(_glob.glob("/persist/v15_phase1_ep*.pt"))
    best_p1 = "/persist/v15_phase1_best.pt"
    final = "/persist/v15_final.pt"
    p1_start_epoch = 0
    best_val_cer = float("inf")
    best_train_ctc = float("inf")

    if Path(final).exists():
        print("✓ V15 already complete.")
        return
    ...
    print(f"\n{'='*60}")
    print(" V15: lexical evidence reranking over the V14 base")
    print(" V14 warm-start + word inventory + count head + filler penalty")
    ...
    torch.save(model.state_dict(), "/persist/v15_final.pt")
    ckpt_vol.commit()
    print("\n✓ V15 complete. Saved /persist/v15_final.pt")


@app.local_entrypoint()
def main(mode: str = "pipeline", epochs_p1: int = 24):
    if mode == "pipeline":
        print(f"Launching V15: Phase1={epochs_p1}ep")
