...
app = modal.App("eeg-vlm-v63")

ckpt_vol = modal.Volume.from_name("bt-checkpoints-v63", create_if_missing=True)
...
# V6.3: real-hint byte-level denoiser on top of V6.1 Phase 1.
...
class EEGCTCEncoder(nn.Module):
    ...
    @torch.no_grad()
    def decode_ctc(self, traj, use_beam=True):
...
class ByT5Denoiser(nn.Module):
    ...
    @torch.no_grad()
    def generate(self, input_ids, attention_mask, max_new_tokens=96):
...
def format_denoiser_source(hint):
    return f"repair eeg text: {(hint or '...').strip()}"
...
class DenoiserPairDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if np.random.random() < self.synth_prob:
            hint = ctc_like_corrupt_text(pair["text"], severity=np.random.uniform(0.8, 1.3))
        return {
            "source": format_denoiser_source(hint),
            "target": pair["text"],
            "hint": pair["hint"],
        }
...
def collate_denoiser(batch, tokenizer, max_source_len=192, max_target_len=192):
    ...
    return enc.input_ids, enc.attention_mask, labels, targets, hints
...
def precompute_real_pairs(ctc_model, subset, split_name):
    ...
    for step, (eegs, texts) in enumerate(loader, start=1):
        hints = ctc_model.decode_ctc(eegs.to(device).to(torch.bfloat16), use_beam=True)
        for hint, text in zip(hints, texts):
            pairs.append({"hint": hint.strip() or "...", "text": text})
        if step % 50 == 0:
...
def load_or_build_pairs(ctc_model, train_subset, val_subset):
    train_cache = Path("/persist/v63_train_pairs.pt")
    val_cache = Path("/persist/v63_val_pairs.pt")
...
def evaluate_denoiser(model, tokenizer, val_pairs, epoch):
    ...
    val_loader = torch.utils.data.DataLoader(
        ...
        collate_fn=lambda b: collate_denoiser(
            [{"source": format_denoiser_source(p["hint"]), "target": p["text"], "hint": p["hint"]} for p in b],
            tokenizer,
        ),
    )
    ...
    shown = 0
    for input_ids, attn_mask, labels, targets, hints in val_loader:
        input_ids = input_ids.to(device)
        attn_mask = attn_mask.to(device)
        out_ids = model.generate(input_ids, attn_mask, max_new_tokens=96)
        preds = tokenizer.batch_decode(out_ids, skip_special_tokens=True)
        for hint, ref, pred in zip(hints, targets, preds):
            ref_lower = ref.lower()
            pred_clean = clean_generated(pred.lower())
...
def train_denoiser(model, tokenizer, train_pairs, val_pairs, ckpt_prefix, epochs, start_epoch=0):
    ...
    for local_epoch in range(1, epochs + 1):
        ...
        for step, batch in enumerate(train_loader, start=1):
            input_ids, attn_mask, labels, _, _ = batch
            input_ids = input_ids.to(device)
            attn_mask = attn_mask.to(device)
...
def run_pipeline(epochs: int = 12):
    ...
    if not load_v61_phase1_encoder(ctc_model):
        raise RuntimeError("V6.1 Phase 1 checkpoint is required for V6.3.")
    ctc_model.eval()
    ...
    ckpt_prefix = "/persist/v63_denoiser"
    final_path = Path("/persist/v63_final.pt")
    epoch_ckpts = sorted(glob.glob("/persist/v63_denoiser_ep*.pt"))
    start_epoch = 0

    if final_path.exists():
        print("V6.3 already complete.")
        return
    ...
    if epoch_ckpts:
        ...
        print(f"Resuming V6.3 denoiser from epoch {start_epoch}")

    print("\n" + "=" * 60)
    print(" V6.3: real-hint ByT5 denoiser on top of V6.1 Phase 1")
