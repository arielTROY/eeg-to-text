...
app = modal.App("eeg-vlm-v7-semanticalock")

ckpt_vol = modal.Volume.from_name("bt-checkpoints-v7", create_if_missing=True)
...
class SpatioTemporalBridge(nn.Module):
    def __init__(self, in_ch=768, out_ch=1024, dropout=0.1):
        super().__init__()
        # V7: Sequence-to-sequence bridge (64 steps) with dropout
...
class EEG_VLM_V7(nn.Module):
    def __init__(self, q_name):
        ...
        v_cfg = VideoMAEConfig(
            num_channels=1, image_size=64, patch_size=16, 
        ...
        lora_cfg = LoraConfig(
            task_type=TaskType.CAUSAL_LM, r=128, lora_alpha=256, # V7: Double rank
    ...
    def forward(self, traj, char_ids, char_lens, input_ids=None, labels=None):
        ...
        if input_ids is not None:
            ...
            # GROUNDING PENALTY: MSE between first N brain tokens and first N text tokens
            # We align the brain sequence to the text sequence in the LLM's embedding space.
            n_align = min(64, tok_embs.shape[1])
            if n_align > 0:
                loss_lock = F.mse_loss(prefix[:, :n_align, :], tok_embs[:, :n_align, :])
...
def train_v7(epochs=30, phase=1):
    ...
    model = EEG_VLM_V7(q_name).to(torch.bfloat16).cuda()
    
    if phase == 2:
        print("--- PHASE 2: Semantic Locking ---")
        # Load Phase 1 weights
        ckpt_path = Path("/persist/v7_phase1_epoch9.pt")
        if ckpt_path.exists():
            print(f"Loading Phase 1 weights: {ckpt_path}")
            sd = torch.load(str(ckpt_path), map_location="cuda")
            # Map bridge.net.3 (Linear) -> bridge.net.4 (Linear) after Dropout addition
            new_sd = {}
            for k, v in sd.items():
                if "bridge.net.3." in k:
                    new_k = k.replace("bridge.net.3.", "bridge.net.4.")
                    new_sd[new_k] = v
                else:
                    new_sd[k] = v
    ...
    print(f"Starting V7 Phase {phase} Training: {len(ds)} samples")
    for epoch in range(1, epochs + 1):
        ...
        for eeg, input_ids, labels, c_ids, c_lens in train_loader:
            ...
            with torch.amp.autocast('cuda', dtype=torch.bfloat16):
                ...
                # V7: Add Lock Penalty in Phase 2
                penalty_weight = 10.0 if phase == 2 else 1.0
                loss = loss_lm + 0.5 * loss_ctc + penalty_weight * loss_lock
        ...
        if epoch % 3 == 0:
            ...
            torch.save(model.state_dict(), f"/persist/v7_phase{phase}_epoch{epoch}.pt")

@app.local_entrypoint()
def main(phase: int = 1, epochs: int = 30):
    train_v7.remote(epochs=epochs, phase=phase)
