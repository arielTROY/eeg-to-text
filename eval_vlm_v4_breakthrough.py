...
app = modal.App("eeg-vlm-v4-1-breakthrough")
...
def run_breakthrough_pass(ckpt_epoch=4):
    import jiwer
    ...
    # 1. CROSS-WORKSPACE STREAMING (Modular)
    needs_data = not os.path.exists(local_data)
    needs_ckpt = not os.path.exists(local_path)
    
    if needs_data or needs_ckpt:
        print(f"Streaming required assets from curspor...")
        old_client = modal.Client.from_credentials("ak-zbkRnV4DZtd1H3fSkjQKtQ", "as-JpYfpc1gA08h3lP55N4aZ3")
        
        if needs_data:
            print("Streaming MindVoice Data...")
        ...
        if needs_ckpt:
            ...
            with open(local_path, "wb") as f:
                for chunk in old_vol.read_file(f"/eeg_vlm_v4_1_epoch{ckpt_epoch}.pt"): f.write(chunk)
            ckpt_vol.commit()
        print("Asset Stream Complete.")

    mv = np.load(local_data, allow_pickle=True)
    mv_texts = [str(t).upper().strip() for t in mv["texts"]] # Syllables were UPPER during training hits
    ...
    class EEGVideoVLM_V4_1_Eval(nn.Module):
        def __init__(self):
            super().__init__()
            self.rasterizer = TopographicRasterizerV4()
            self.ch_expand = nn.Conv2d(1, 3, 1)
            self.video_enc = VideoMAEModel(vmae_cfg)
            self.bridge = CrossAttentionBridge(1024, n_heads=16)
        ...
        def generate(self, traj, tokenizer):
            ...
            # HIGH FIDELITY BEAM SEARCH (num_beams=5)
    ...
    for idx in val_idx[:20]:
        ...
        # Clean the pred - model often outputs things like "[EEG: ] TE"
        pred = raw_pred.split("[EEG: ]")[-1].strip().upper() if "[EEG: ]" in raw_pred else raw_pred.strip().upper()
        # Handle trailing space hallucinations
    ...
    acc = np.mean([r["exact"] for r in results])
    print(f"\nBreakthrough Report (Epoch {ckpt_epoch}): ACC: {acc:.4f}")
    return {"epoch": ckpt_epoch, "accuracy": float(acc)}

@app.local_entrypoint()
def main(epoch: int = 4):
    run_breakthrough_pass.remote(ckpt_epoch=epoch)
