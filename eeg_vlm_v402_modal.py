...
app = modal.App("eeg-vlm-v402-audio-ctc")

ckpt_vol = modal.Volume.from_name("bt-checkpoints-v402", create_if_missing=True)
...
# V402: grounded audio-family branch.
# EEG timeline -> conv frontend -> temporal transformer -> pseudo-mel
# -> Whisper encoder -> char + small wordpiece CTC heads.
...
def build_sentencepiece_model(texts, model_prefix="/persist/v402_wordpiece", vocab_size=192):
...
class EEGDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        s = self.samples[idx]
        return s["eeg"], s["text"], text_to_char_ids(s["text"]), text_to_wp_ids(s["text"])


def collate_fn(batch):
    eegs, texts, char_ids_list, wp_ids_list = zip(*batch)
    ...
    return eegs, list(texts), char_pad, char_lens, wp_pad, wp_lens


class EEGAudioWhisperCTC(nn.Module):
    def __init__(self):
        super().__init__()
        ...
        self.char_head = nn.Linear(self.whisper_encoder.config.d_model, VOCAB_SIZE)
        self.wp_head = nn.Linear(self.whisper_encoder.config.d_model, WP_VOCAB_SIZE)
        self.char_loss_fn = nn.CTCLoss(blank=0, zero_infinity=True)
    ...
    def forward(self, traj, char_ids, char_lens, wp_ids, wp_lens):
        states = self.encode_states(traj)
        char_logits = self.char_head(states).transpose(0, 1)
        ...
        loss_char = self.char_loss_fn(F.log_softmax(char_logits, dim=-1), char_ids, input_lens, char_lens)
        loss_wp = self.wp_loss_fn(F.log_softmax(wp_logits, dim=-1), wp_ids, input_lens, wp_lens)
        return {"loss_char": loss_char, "loss_wp": loss_wp}
    ...
    def decode(self, traj, nbest=5):
        ...
        char_logits = self.char_head(states).transpose(0, 1)
        wp_logits = self.wp_head(states).transpose(0, 1)
        char_groups = beam_ctc_decode_nbest(char_logits, beam_width=12, nbest=nbest, char_lm=CHAR_LM)
        wp_groups = beam_ctc_decode_wp_nbest(wp_logits, beam_width=12, nbest=nbest)
        merged = []
        for char_beams, wp_beams in zip(char_groups, wp_groups):
            merged.append(merge_char_wp_candidates(char_beams, wp_beams, nbest=nbest))
        return merged, char_groups, wp_groups
...
def set_trainable(model, freeze_whisper_body):
    ...
    for p in model.to_mel.parameters():
        p.requires_grad = True
    for p in model.char_head.parameters():
        p.requires_grad = True
...
def make_optimizer(model, steps_per_epoch, epochs, freeze_whisper_body):
    ...
    add(model.char_head.parameters(), 2e-4)
...
def validate(model, val_loader, epoch, ckpt_prefix, best_wer):
    ...
    with torch.no_grad():
        shown = 0
        for eegs, texts, _, _, _, _ in val_loader:
            merged, char_groups, wp_groups = model.decode(eegs.to(device).to(torch.bfloat16), nbest=5)
            for merged_beams, char_beams, wp_beams, ref in zip(merged, char_groups, wp_groups, texts):
                ...
                if shown < 4:
                    ...
                    print(f"  CHR: '{char_beams[0][0][:120] if char_beams else ''}'")
...
def train_stage(model, epochs, train_loader, val_loader, ckpt_prefix, start_epoch, best_wer, freeze_whisper_body, stage_name):
    ...
    for local_epoch in range(1, epochs + 1):
        ...
        for step, batch in enumerate(train_loader):
            eegs, _, char_ids, char_lens, wp_ids, wp_lens = batch
            ...
            with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                out = model(eegs, char_ids, char_lens, wp_ids, wp_lens)
                loss = (4.0 * out["loss_wp"] + 2.5 * out["loss_char"]) / ACCUM_STEPS
            loss.backward()
            tot_char += out["loss_char"].item()
        ...
        print(f"Epoch {epoch:3d} | WPC:{tot_wp/n:.3f} CCTC:{tot_char/n:.3f} | LR0:{lr:.2e}")
...
def run_pipeline(epochs_p1: int = 2):
    ...
    global SP_MODEL, WP_VOCAB_SIZE, CHAR_LM
    train_texts = [ds.samples[i]["text"] for i in train_idx]
    CHAR_LM = CharNGramLM(train_texts, order=3)
    SP_MODEL = build_sentencepiece_model(train_texts, model_prefix="/persist/v402_wordpiece", vocab_size=192)
    ...
    model = EEGAudioWhisperCTC().to(torch.bfloat16).cuda()

    print("\n============================================================")
    print(" V402: EEG timeline -> audio frontend -> Whisper encoder -> char/wp CTC")
    print(" FRESH START")
    print("============================================================\n")
    ...
    best_path = Path("/persist/v402_best.pt")
    if best_path.exists():
        model.load_state_dict(torch.load(best_path, map_location="cuda"))
    torch.save(model.state_dict(), "/persist/v402_final.pt")
    ckpt_vol.commit()
    print("\n✓ V402 complete. Saved /persist/v402_final.pt")


@app.local_entrypoint()
def main(epochs_p1: int = 2):
    print(f"Launching V402: Phase1={epochs_p1}ep")
