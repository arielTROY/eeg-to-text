...
app = modal.App("eeg-vlm-v42")

ckpt_vol = modal.Volume.from_name("bt-checkpoints-v42", create_if_missing=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
# V4.2: Recover the missing V20 lexical signal, then keep the stronger V41
# Phase 2 path.
#
# Root cause of V4.1 stalling:
#   The model no longer had the full V20 Phase 1 recipe. V20 learned on
#   sentence-level ZuCo plus word-level InnerSpeech, and its best checkpoint was
#   trained with V20's exact preprocessing. V4.1 dropped InnerSpeech and changed
#   the smoothing path, so even the better V4.0 MAE encoder still plateaued near
#   CTC 3.13 with mostly vowel skeletons.
#
# Fix:
#   [M1] Warm-start Phase 1 from the proven V20 checkpoint
#   [M2] Restore V20 preprocessing exactly (gamma + cross-channel smoothing)
#   [M3] Restore InnerSpeech as train-only lexical support for Phase 1
#   [M4] Keep the V41 Phase 2 denoising stack unchanged
#
# All useful improvements kept:
#   [B1] Gamma band (30-100Hz)
#   [B2] Character-only CTC in Phase 1
...
def load_v20_phase1_warmstart(model, ckpt_path="/v20/v20_phase1_best.pt"):
    """Load the proven V20 Phase 1 alignment stack into the current model."""
...
def _train_phase(model, phase, epochs, train_loader, val_loader,
    ...
    if phase == 1:
        print("── Phase 1: V20 warm-start + char CTC + lexical support ──")
...
def run_pipeline(epochs_p1: int = 18, epochs_p2: int = 40):
    ...
    if len(ds_inner) > 0:
        ...
        train_ds = torch.utils.data.ConcatDataset([train_zuco, train_inner])
    ...
    model = EEG_VLM_V42(q_name).to(torch.bfloat16).cuda()

    # ── Resume logic ──────────────────────────────────────────────────────────
    import glob as _glob
    p1_ckpts  = sorted(_glob.glob("/persist/v42_phase1_ep*.pt"))
    p2_ckpts  = sorted(_glob.glob("/persist/v42_phase2_ep*.pt"))
    best_p1   = "/persist/v42_phase1_best.pt"
    final     = "/persist/v42_final.pt"

    if Path(final).exists():
        print("✓ V4.2 already complete.")
        return
    ...
    print(f"\n{'='*60}")
    print(f" V4.2: V20 warm-start + InnerSpeech Phase 1 + V41 Phase 2")
    ...
    if not skip_p1:
        _train_phase(model, phase=1, epochs=epochs_p1,
                     ...
                     tokenizer=tokenizer, ckpt_prefix="/persist/v42")
    ...
    torch.save(model.state_dict(), "/persist/v42_final.pt")
    ckpt_vol.commit()
    print("\n✓ V4.2 complete. Saved /persist/v42_final.pt")


@app.local_entrypoint()
def main(mode: str = "pipeline", epochs_p1: int = 18, epochs_p2: int = 40):
    if mode == "pipeline":
        print(f"Launching V4.2: Phase1={epochs_p1}ep, Phase2={epochs_p2}ep")
