...
app = modal.App("eeg-vlm-v100-final")

ckpt_vol = modal.Volume.from_name("bt-checkpoints-v100", create_if_missing=True)
...
#
# Add:
#   [1] exact scalp-geometry rasterization with correct x/y orientation
#   [2] two-view video input: full scalp + left-hemisphere language-focused map
#   [3] phone-aware local reranking from phone CTC posteriors
#   [4] short two-stage schedule: frozen early alignment, then light adaptation
#
# The goal is to maximize useful beam contents fast, not to optimize a surrogate
# objective that later destroys consonant and chunk structure.
# ═══════════════════════════════════════════════════════════════════════════════
...
def run_pipeline(epochs_p1: int = 3, use_innerspeech: bool = False):
    ...
    SP_MODEL = build_sentencepiece_model(train_texts_for_vocab, model_prefix="/persist/v100_wordpiece", vocab_size=192)
    WP_VOCAB_SIZE = SP_MODEL.get_piece_size() + 1
    print(f"[V100] ngram vocab size: {len(NGRAM_LIST)}")
    print(f"[V100] wordpiece vocab size: {WP_VOCAB_SIZE - 1}")
    ...
    p1_ckpts = sorted(_glob.glob("/persist/v100_phase1_ep*.pt"))
    best_p1 = "/persist/v100_phase1_best.pt"
    final = "/persist/v100_final.pt"
    p1_start_epoch = 0
    best_phono_top1 = -1.0
    best_oracle_wer = float("inf")
    ...
    print(f"\n{'='*60}")
    print(" V100: two-view topographic video + local phone timeline")
    print(" V17 warm-start + full/left scalp maps + wordpiece/char/phone CTC")
    ...
    torch.save(model.state_dict(), "/persist/v100_final.pt")
    ckpt_vol.commit()
    print("\n✓ V100 complete. Saved /persist/v100_final.pt")


@app.local_entrypoint()
def main(mode: str = "pipeline", epochs_p1: int = 3, use_innerspeech: bool = False):
    if mode == "pipeline":
        print(f"Launching V100: Phase1={epochs_p1}ep inner={use_innerspeech}")
