...
app      = modal.App("eeg-vlm-v700")
ckpt_vol = modal.Volume.from_name("bt-checkpoints-v700", create_if_missing=True)
...
class EEGV700(nn.Module):
    ...
    def forward(self, traj, sbert_embs, target_ids, target_mask,
        ...
        total = 1.0 * loss_lm + 0.5 * loss_align + 0.20 * loss_sent + 0.10 * loss_len
        return total, loss_lm, loss_align, loss_sent, loss_len
...
def load_warm_start(model):
    """Try warm-starting VideoMAE from V61 checkpoint."""
    import glob
    v61_dir = Path("/v61_ckpt")
    ckpts   = sorted(glob.glob(str(v61_dir / "*.pt")))
    if not ckpts:
        print("No V61 checkpoint found, training from scratch.")
        return
    ...
    loaded, skipped = [], []
    for k, v in sd.items():
        mapped = k.replace("video_enc.", "video_enc.")
        if mapped in own and own[mapped].shape == v.shape:
            own[mapped] = v; loaded.append(mapped)
        else:
            skipped.append(k)
    model.load_state_dict(own, strict=False)
    print(f"Warm-start: {len(loaded)} params loaded, {len(skipped)} skipped.")
    ...
    for epoch in range(1, epochs + 1):
        ...
        for step, batch in enumerate(train_loader, 1):
            ...
            with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                total, lm, align, sent_l, len_l = model(
            ...
            if n_accum % ACCUM_STEPS == 0:
                ...
                if (n_accum // ACCUM_STEPS) % 10 == 0:
                    print(f"  ep={epoch} step={n_accum//ACCUM_STEPS} "
                          ...
                          f"sent={sent_l.item():.3f} len={len_l.item():.3f}")
        ...
        if epoch % 5 == 0 or epoch == 1:
            ...
            improved = wer < best_wer or cr > best_cr
            if improved:
                best_wer = min(best_wer, wer)
                best_cr  = max(best_cr,  cr)
                ckpt_path = ckpt_dir / f"v700_ep{epoch}_wer{wer:.3f}.pt"
                torch.save({
                    "epoch": epoch, "wer": wer, "cr": cr,
