...
app = modal.App("eeg-vlm-v13-ctc")

ckpt_vol = modal.Volume.from_name("bt-checkpoints-v13", create_if_missing=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
# V13: V12 plus consonant inventory supervision.
#
# What failed:
...
#
# This branch keeps the working V12 path intact and adds one lightweight
# sentence-level objective: predict which consonants are present anywhere in the
# sentence. That targets the exact remaining failure mode without perturbing the
# time-alignment path much.
# ═══════════════════════════════════════════════════════════════════════════════
...
def beam_ctc_decode_nbest(logits, beam_width=10, nbest=3, blank_penalty=0.20):
...
class EEG_CTC_V13(nn.Module):
    ...
    def forward(self, traj, char_ids, char_lens, class_ids, class_lens, cons_targets):
        B = traj.shape[0]
        device = traj.device
        ...
        cons_logits = self.cons_head(v_seq.mean(dim=1))
...
def load_v12_phase1_warmstart(model, ckpt_path="/v12/v12_phase1_best.pt"):
    if not Path(ckpt_path).exists():
        print(f"V12 Phase 1 checkpoint not found at {ckpt_path}")
        return False

    print(f"Loading V12 Phase 1 warm-start weights from {ckpt_path}...")
    src_sd = torch.load(ckpt_path, map_location="cpu")
    dst_sd = model.state_dict()
    prefixes = ("rasterizer.", "video_enc.", "ch_adapt.", "ctc_head.", "class_ctc_head.")
    n_copied = n_skipped = 0
    for k, v in src_sd.items():
    ...
    print(f"  ✓ Loaded {n_copied} V12 Phase 1 tensors, skipped {n_skipped}")
    return n_copied > 0
...
class EEGDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return eeg, s['text'], char_ids, class_ids, cons_target


def collate_fn(batch):
    eegs, texts, char_ids_list, class_ids_list, cons_targets = zip(*batch)
    ...
    return eegs, list(texts), char_pad, char_lens, class_pad, class_lens, cons_tensor
...
def _validate_phase1(model, val_loader, epoch, ckpt_prefix, best_val_cer):
    ...
    with torch.no_grad():
        ...
        for batch in val_loader:
            eeg_v, texts_v, _, _, _, _, _ = batch
            beam_groups = model.decode_ctc(eeg_v.to(device).to(torch.bfloat16), beam_width=12, nbest=3)
...
def _train_phase1_stage(model, epochs, train_loader, val_loader, ckpt_prefix,
    ...
    for local_epoch in range(1, epochs + 1):
        ...
        model.train()
        tot_ctc = tot_class = tot_cons = tot_div = 0.0
        optimizer.zero_grad()

        for step, batch in enumerate(train_loader):
            eeg, _, c_ids, c_lens, class_ids, class_lens, cons_targets = batch
            ...
            with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                out = model(eeg, c_ids, c_lens, class_ids, class_lens, cons_targets)
                loss = (
                    10.0 * out["loss_ctc"] +
                    0.8 * out["loss_class"] +
                    0.35 * out["loss_cons"] +
        ...
        print(
            ...
            f"CONS:{tot_cons/n:.3f} Div:{tot_div/n:.4f} | LR0:{lr:.2e}"
        )
...
@app.function(image=image, gpu="H100", timeout=86400,
              volumes={"/data": data_vol, "/persist": ckpt_vol, "/v20": v20_vol, "/v50": v50_vol, "/v12": v12_vol},
...
def run_pipeline(epochs_p1: int = 24):
    ...
    model = EEG_CTC_V13().to(torch.bfloat16).cuda()

    # ── Resume logic ──────────────────────────────────────────────────────────
    import glob as _glob
    p1_ckpts = sorted(_glob.glob("/persist/v13_phase1_ep*.pt"))
    best_p1 = "/persist/v13_phase1_best.pt"
    final = "/persist/v13_final.pt"
    p1_start_epoch = 0
    best_val_cer = float("inf")
    best_train_ctc = float("inf")

    if Path(final).exists():
        print("✓ V13 already complete.")
        return
    ...
    if Path(best_p1).exists() and p1_ckpts:
        ...
        if not load_v12_phase1_warmstart(model):
    ...
    print(f"\n{'='*60}")
    print(" V13: conservative CTC + consonant inventory")
    print(" V12 warm-start + ACTC + sentence-level consonant support")
    ...
    torch.save(model.state_dict(), "/persist/v13_final.pt")
    ckpt_vol.commit()
    print("\n✓ V13 complete. Saved /persist/v13_final.pt")


@app.local_entrypoint()
def main(mode: str = "pipeline", epochs_p1: int = 24):
    if mode == "pipeline":
        print(f"Launching V13: Phase1={epochs_p1}ep")
