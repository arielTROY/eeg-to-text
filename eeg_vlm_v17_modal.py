...
app = modal.App("eeg-vlm-v17-ctc")

ckpt_vol = modal.Volume.from_name("bt-checkpoints-v17", create_if_missing=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
# V17: hierarchical decoding with wordpiece CTC as the main unit.
#
...
#   [1] main wordpiece CTC head
#   [2] auxiliary char CTC head
#   [3] boundary head over the encoder timeline
#   [4] joint decode from char + wordpiece candidates
# The intent is to move from isolated letters toward actual lexical chunks.
# ═══════════════════════════════════════════════════════════════════════════════
...
def _train_phase1_stage(model, epochs, train_loader, val_loader, ckpt_prefix,
    ...
    for local_epoch in range(1, epochs + 1):
        ...
        model.train()
        tot_char = tot_wp = tot_class = tot_cons = tot_ngram = tot_bnd = tot_div = 0.0
        optimizer.zero_grad()
        ...
        for step, batch in enumerate(train_loader):
            ...
            with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                ...
                loss = (
                    10.0 * out["loss_wp_ctc"] +
                    4.0 * out["loss_char_ctc"] +
                    0.6 * out["loss_class"] +
                    0.35 * out["loss_cons"] +
                    0.30 * out["loss_ngram"] +
...
def run_phase1_curriculum(model, epochs_p1, train_loader_mix, train_loader_pure,
    ...
    if current_epoch < mix_epochs and train_loader_mix is not None:
        ...
        print(f"[Phase1] Stage A: frozen wordpiece alignment for {stage_epochs} epochs")
...
def run_pipeline(epochs_p1: int = 24):
    ...
    SP_MODEL = build_sentencepiece_model(train_texts_for_vocab, model_prefix="/tmp/v17_wordpiece", vocab_size=192)
    WP_VOCAB_SIZE = SP_MODEL.get_piece_size() + 1
    print(f"[V17] ngram vocab size: {len(NGRAM_LIST)}")
    print(f"[V17] wordpiece vocab size: {WP_VOCAB_SIZE - 1}")

    model = EEG_CTC_V17(ngram_dim=len(NGRAM_LIST), wp_vocab_size=WP_VOCAB_SIZE).to(torch.bfloat16).cuda()
    model.char_lm = CharNGramLM(train_texts_for_vocab, order=3)

    # ── Resume logic ──────────────────────────────────────────────────────────
    import glob as _glob
    p1_ckpts = sorted(_glob.glob("/persist/v17_phase1_ep*.pt"))
    best_p1 = "/persist/v17_phase1_best.pt"
    final = "/persist/v17_final.pt"
    p1_start_epoch = 0
    best_val_cer = float("inf")
    best_train_ctc = float("inf")

    if Path(final).exists():
        print("✓ V17 already complete.")
        return
    ...
    print(f"\n{'='*60}")
    print(" V17: wordpiece CTC main + char CTC auxiliary")
    print(" V14 warm-start + joint char/wordpiece decode + boundary head")
    ...
    torch.save(model.state_dict(), "/persist/v17_final.pt")
    ckpt_vol.commit()
    print("\n✓ V17 complete. Saved /persist/v17_final.pt")


@app.local_entrypoint()
def main(mode: str = "pipeline", epochs_p1: int = 24):
    if mode == "pipeline":
        print(f"Launching V17: Phase1={epochs_p1}ep")
