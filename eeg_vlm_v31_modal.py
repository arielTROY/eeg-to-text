...
app = modal.App("eeg-vlm-v31")

ckpt_vol = modal.Volume.from_name("bt-checkpoints-v31b", create_if_missing=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
# V3.1b FINAL: Character CTC w/ Consonant Boosting + CTC-Guided Denoising
#
# Key insight: Subword CTC (80 or 300 tokens) plateaus at high loss because
# EEG temporal resolution can't discriminate word-pieces. Character CTC (40
# tokens) works — V2.0 achieved 1.489 loss and decoded vowel skeletons.
# But vowels-only isn't enough for word reconstruction.
#
# Solution: Character CTC + consonant-weighted auxiliary loss.
# The model learns vowels naturally (strong EEG signatures). We add a
# per-frame focal loss that upweights consonant predictions 3x, forcing
# gradient pressure on the harder characters.
#
# Combined with V3.1 bug fixes (theta band, per-channel smoothing, padding
# mask, augmentation) and Phase 2 improvements (CTC-guided denoising,
# hint curriculum, prefix dropout ramp, chat template, beam decode).
#
# BUGS FIXED:
#   [B1] Gamma band → theta (4-8Hz, language-relevant, valid Nyquist)
#   [B2] Per-channel temporal smoothing (no cross-channel bleed)
#   [B3] LM padding tokens masked as -100
#   [B4] No OOV issue — full character vocab covers all text
# ═══════════════════════════════════════════════════════════════════════════════

# ─── Character Vocabulary + Consonant Weights ────────────────────────────────
...
def build_char_weights():
    """Per-character weights: 3x for consonants, 1x for vowels/blank/punct."""
    w = torch.ones(VOCAB_SIZE)
    for c, idx in CHAR_TO_ID.items():
        if c in CONSONANTS:
            w[idx] = 3.0
        elif c in VOWELS:
            w[idx] = 1.0
        elif c == ' ':
            w[idx] = 1.5  # spaces help word boundary detection
    return w
...
def augment_eeg(eeg_np, strength='moderate'):
    ...
    if strength == 'moderate':
        jitter, amp_lo, amp_hi, noise_std, ch_drop_p, mask_spans = (
            64, 0.85, 1.15, 0.05, 0.10, 2)
    else:
        jitter, amp_lo, amp_hi, noise_std, ch_drop_p, mask_spans = (
            16, 0.95, 1.05, 0.02, 0.05, 1)
...
class EEG_VLM_V31(nn.Module):
    def __init__(self, q_name):
        ...
        # Consonant-weighted auxiliary cross-entropy (per-frame)
        char_weights = build_char_weights()
    ...
    def forward(self, traj, char_ids, char_lens,
        ...
        # Consonant-boosted auxiliary loss: per-frame weighted CE
        # This gives direct gradient to consonant character predictions
        # We create frame-level targets by stretching char_ids across 256 frames
        with torch.no_grad():
            # Simple uniform stretch of characters across frames
            frame_targets = torch.zeros(B, 256, dtype=torch.long, device=device)
            for b_idx in range(B):
                n_chars = char_lens[b_idx].item()
                if n_chars > 0:
                    chars = char_ids[b_idx, :n_chars]
                    # Map each char to its proportional frame position
                    positions = torch.linspace(0, 255, n_chars, device=device).long()
                    frame_targets[b_idx, positions] = chars

        # Weighted CE: consonants get 3x, vowels 1x
        logits_flat = ctc_logits.transpose(0, 1).reshape(-1, VOCAB_SIZE)  # (B*256, V)
        targets_flat = frame_targets.reshape(-1)  # (B*256,)
        loss_consonant = F.cross_entropy(
            logits_flat, targets_flat,
            weight=self.char_weights.to(device=device, dtype=logits_flat.dtype),
            ignore_index=0)  # ignore blank frames
        ...
        if input_ids is None:
            ...
            return zero, loss_ctc, zero, loss_div, None, loss_consonant
        ...
        return lm_out.loss, loss_ctc, loss_lock, loss_div, tok_mean_raw, loss_consonant
...
def normalize_eeg(eeg, target_ch=64, target_t=1024):
    ...
    combined = np.concatenate([eeg.T, theta.T, alpha.T, beta.T], axis=1)
...
def _train_phase(model, phase, epochs, train_loader, val_loader,
    ...
    if phase == 1:
        print("── Phase 1: Character CTC + Consonant Boost ──")
    ...
    for epoch in range(1, epochs + 1):
        model.train()
        tot_lm = tot_ctc = tot_lock = tot_div = tot_cons = 0.0
        optimizer.zero_grad()
        ...
        for step, batch in enumerate(train_loader):
            ...
            with torch.amp.autocast('cuda', dtype=torch.bfloat16):
                if phase == 1:
                    _, loss_ctc, _, loss_div, _, loss_cons = model(eeg, c_ids, c_lens)
                    # CTC + consonant auxiliary + diversity
                    loss = (10.0 * loss_ctc + 3.0 * loss_cons + 0.5 * loss_div) / ACCUM_STEPS
                    ...
                    loss_lm, loss_ctc, loss_lock, loss_div, tok_mean, loss_cons = model(
                    ...
                    loss = (loss_lm + 1.0 * loss_ctc + 1.0 * loss_cons
        ...
        print(f"Epoch {epoch:3d} | LM:{tot_lm/n:.3f} CTC:{tot_ctc/n:.3f} "
              f"Cons:{tot_cons/n:.3f} Lock:{tot_lock/n:.4f} Div:{tot_div/n:.4f} | LR:{lr:.2e}")
        ...
        if epoch % 3 == 0:
            ...
            # Track vowel vs consonant accuracy separately
            ...
            with torch.no_grad():
                ...
                for batch in val_loader:
                    ...
                    eeg_dev = eeg_v.to(device)
                    llm_preds, ctc_preds = model.generate(
                        eeg_dev, tokenizer, use_beam=(phase == 2))
...
def run_pipeline(epochs_p1: int = 40, epochs_p2: int = 40):
    ...
    ds_train = EEGDataset("/data/EEG_Text", augment='moderate')
    ...
    model = EEG_VLM_V31(q_name).to(torch.bfloat16).cuda()

    # Resume logic
    import glob as _glob
    p1_ckpts = sorted(_glob.glob("/persist/v31b_phase1_ep*.pt"))
    p2_ckpts = sorted(_glob.glob("/persist/v31b_phase2_ep*.pt"))
    best_p1 = "/persist/v31b_phase1_best.pt"
    final = "/persist/v31b_final.pt"

    if Path(final).exists():
        print("✓ V3.1b already complete.")
        return
    ...
    if p2_ckpts:
        ...
        if last_p1_epoch >= epochs_p1 - 3:
            ...
            print(f"✓ Phase 1 done. Loading best.")
            skip_p1 = True
        else:
    ...
    print(f"\n{'='*60}")
    print(f" V3.1b FINAL: Char CTC + Consonant Boost + Denoising")
    print(f" {'(RESUMED)' if skip_p1 else 'FRESH START'}")
    print(f"{'='*60}\n")

    if not skip_p1:
        _train_phase(model, phase=1, epochs=epochs_p1,
                     train_loader=train_loader, val_loader=val_loader,
                     tokenizer=tokenizer, ckpt_prefix="/persist/v31b")
    ...
    torch.save(model.state_dict(), "/persist/v31b_final.pt")
    ckpt_vol.commit()
    print("\n✓ V3.1b FINAL complete. Saved /persist/v31b_final.pt")


@app.local_entrypoint()
def main(mode: str = "pipeline", epochs_p1: int = 40, epochs_p2: int = 40):
    if mode == "pipeline":
        print(f"Launching V3.1b FINAL: Phase1={epochs_p1}ep, Phase2={epochs_p2}ep")
