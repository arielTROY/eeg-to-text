...
app = modal.App("eeg-vlm-v20x-semantic")

ckpt_vol = modal.Volume.from_name("bt-checkpoints-v20x", create_if_missing=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
# V20x: semantic-aligned open-text decoding.
#
# What failed recently:
#   [1] decoder-only changes did not move oracle WER enough
#   [2] lexical memory / RNNT / reranking all hit the same beam ceiling
#   [3] V17 briefly improved oracle WER, which suggests the encoder needs
#       stronger sentence-level separation before any decoder can exploit it
#
# What worked:
#   [1] V17 had the best early beam quality
#   [2] the EEG-as-video path is recovering local chunks and some consonants
#
# This branch keeps the open-vocab decode path from V17 and adds:
#   [1] sentence-level semantic alignment to fixed text teacher embeddings
#   [2] memory-bank negatives so batch size 4 is not the ceiling
#   [3] retrieval metrics on train-text semantics, but only as evaluation
#       and auxiliary structure, not the final output path
# The goal is to make the encoder sentence-discriminative while preserving an
# open-text decoder for later real-world use.
# ═══════════════════════════════════════════════════════════════════════════════
...
def build_sentencepiece_model(texts, model_prefix="/tmp/v17_wordpiece", vocab_size=192):
...
def build_text_embedding_lookup(texts):
    from sentence_transformers import SentenceTransformer

    unique = sorted({canonicalize_text(text) for text in texts if canonicalize_text(text)})
    if not unique:
        return {}, [], torch.empty(0, SEMANTIC_DIM, dtype=torch.float32)
    ...
    lookup = {text: embs[i] for i, text in enumerate(unique)}
    return lookup, unique, torch.from_numpy(embs)
...
def merge_char_wp_candidates(char_beams, wp_beams, ngram_probs, cons_probs, boundary_probs, nbest=5):
...
class EEG_CTC_V20X(nn.Module):
    def __init__(self, ngram_dim, wp_vocab_size):
        super().__init__()
    ...
    def forward(self, traj, char_ids, char_lens, wp_ids, wp_lens, class_ids, class_lens,
                cons_targets, ngram_targets, boundary_targets, text_embs=None,
                neg_text_bank=None):
        B = traj.shape[0]
        device = traj.device
        ...
        if text_embs is not None:
            eeg_sem = F.normalize(self.contrast_proj(pooled), dim=-1)
            text_sem = F.normalize(text_embs, dim=-1)
            if neg_text_bank is not None and neg_text_bank.shape[0] > 0:
                all_text = torch.cat([text_sem, neg_text_bank.to(device=device, dtype=eeg_sem.dtype)], dim=0)
                sims = eeg_sem @ all_text.T / 0.07
                targets = torch.arange(B, device=device)
                loss_sem = F.cross_entropy(sims, targets)
            else:
                sims = eeg_sem @ text_sem.T / 0.07
                targets = torch.arange(B, device=device)
                loss_sem = (F.cross_entropy(sims, targets) + F.cross_entropy(sims.T, targets)) / 2.0
            loss_sem = loss_sem + 0.5 * (1.0 - torch.sum(eeg_sem * text_sem, dim=-1).mean())
        else:
    ...
    def decode_ctc(self, traj, beam_width=16, nbest=5):
        ...
        merged = []
        for char_beams, wp_beams, ngp, cp, bp in zip(char_groups, wp_groups, ngram_probs, cons_probs, boundary_probs):
            merged.append(merge_char_wp_candidates(char_beams, wp_beams, ngp, cp, bp, nbest=nbest))
        return merged

    @torch.no_grad()
    def semantic_scores(self, traj):
        traj = traj.to(next(self.parameters()).dtype)
        pooled = self.encode(traj).mean(dim=1)
        eeg_sem = F.normalize(self.contrast_proj(pooled), dim=-1)
        if RETRIEVAL_EMBS is None or RETRIEVAL_EMBS.numel() == 0:
            return torch.empty((traj.shape[0], 0), device=traj.device)
        bank = RETRIEVAL_EMBS.to(device=traj.device, dtype=eeg_sem.dtype)
        return eeg_sem @ bank.T
...
class EEGDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return eeg, s['text'], char_ids, wp_ids, class_ids, cons_target, ngram_target, boundary_target


def collate_fn(batch):
    eegs, texts, char_ids_list, wp_ids_list, class_ids_list, cons_targets, ngram_targets, boundary_targets = zip(*batch)
    ...
    text_embs = np.stack([
        TEXT_EMB_LOOKUP.get(canonicalize_text(text), np.zeros(SEMANTIC_DIM, dtype=np.float32))
        for text in texts
    ]).astype(np.float32)
    text_emb_tensor = torch.from_numpy(text_embs)
    return (
        eegs, list(texts), char_pad, char_lens, wp_pad, wp_lens, class_pad, class_lens,
        cons_tensor, ngram_tensor, boundary_tensor, text_emb_tensor,
    )
...
def set_phase1_trainable(model, freeze_encoder):
    ...
    for p in model.cons_head.parameters():
        p.requires_grad = True
    for p in model.contrast_proj.parameters():
        p.requires_grad = True
...
def make_phase1_optimizer(model, steps_per_epoch, epochs, freeze_encoder):
    ...
    add_group(model.contrast_proj.parameters(), 1.2e-4 if freeze_encoder else 1.5e-4, "sem")
...
def _validate_phase1(model, val_loader, epoch, ckpt_prefix, best_match_top1, best_oracle_wer):
    device = next(model.parameters()).device
    model.eval()
    cer_scores, wer_scores = [], []
    oracle_cer_scores, oracle_wer_scores, diversity_scores = [], [], []
    retrieval_wer_scores = []
    match_top1 = match_top5 = match_top10 = match_covered = 0
    vowel_correct = vowel_total = cons_correct = cons_total = 0
    print(f"── Val Epoch {epoch} ──")
    with torch.no_grad():
        shown = 0
        for batch in val_loader:
            eeg_v, texts_v, _, _, _, _, _, _, _, _, _, _ = batch
            beam_groups = model.decode_ctc(eeg_v.to(device).to(torch.bfloat16), beam_width=16, nbest=5)
            retrieval_scores = model.semantic_scores(eeg_v.to(device).to(torch.bfloat16))
            topk = min(10, retrieval_scores.shape[1]) if retrieval_scores.ndim == 2 else 0
            top_ids = (
                retrieval_scores.topk(topk, dim=-1).indices.detach().cpu().tolist()
                if topk > 0 else [[] for _ in texts_v]
            )
            for beams, retrieved_ids, ref in zip(beam_groups, top_ids, texts_v):
                ref_lower = ref.lower()
                ...
                retrieved_text = RETRIEVAL_TEXTS[retrieved_ids[0]] if retrieved_ids else ""
                ...
                if retrieved_text:
                    retrieval_wer_scores.append(compute_wer(retrieved_text, ref_lower))
                ref_id = RETRIEVAL_TEXT_TO_ID.get(ref_key, -1)
                if ref_id >= 0:
                    match_covered += 1
                ...
                if len(ref.split()) > 3 or shown < 2:
                    ...
                    if retrieved_text:
                        print(f"  RET1: '{retrieved_text[:100]}'")
                    if len(beam_texts) > 1:
    ...
    match_top1_acc = (match_top1 / match_covered) if match_covered else 0.0
    if cer_scores:
        ...
        if match_covered:
            print(
                f"  Retrieval@1: {match_top1/match_covered:.3f}  "
                f"@5: {match_top5/match_covered:.3f}  @10: {match_top10/match_covered:.3f}  "
                f"Coverage: {match_covered}/{len(cer_scores)}"
            )
        if retrieval_wer_scores:
            print(f"  Retrieval top1 WER: {np.mean(retrieval_wer_scores):.3f}")
    ...
    ckpt_vol.commit()
    if (match_top1_acc > best_match_top1) or (
        abs(match_top1_acc - best_match_top1) < 1e-9 and mean_oracle_wer < best_oracle_wer
    ):
        best_match_top1 = match_top1_acc
        best_oracle_wer = mean_oracle_wer
        torch.save(model.state_dict(), f"{ckpt_prefix}_phase1_best.pt")
        print(f"  ✓ New best semantic checkpoint: R@1={best_match_top1:.3f} OracleWER={best_oracle_wer:.3f}")
    return best_match_top1, best_oracle_wer


def _train_phase1_stage(model, epochs, train_loader, val_loader, ckpt_prefix,
                        epoch_offset=0, best_train_ctc=float("inf"),
                        best_match_top1=-1.0, best_oracle_wer=float("inf"), freeze_encoder=False,
                        stage_name=""):
    device = next(model.parameters()).device
    optimizer, scheduler, groups = make_phase1_optimizer(
        model, len(train_loader), epochs, freeze_encoder=freeze_encoder)
    trainable = [p for p in model.parameters() if p.requires_grad]
    sem_bank = MemoryBank(size=2048, dim=SEMANTIC_DIM)
    ...
    for local_epoch in range(1, epochs + 1):
        ...
        model.train()
        tot_char = tot_wp = tot_class = tot_cons = tot_sem = tot_ngram = tot_bnd = tot_div = 0.0
        optimizer.zero_grad()

        for step, batch in enumerate(train_loader):
            eeg, _, c_ids, c_lens, wp_ids, wp_lens, class_ids, class_lens, cons_targets, ngram_targets, boundary_targets, text_embs = batch
            ...
            text_embs = text_embs.to(device=device, dtype=torch.float32)
            neg_text_bank = sem_bank.get(device)

            with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                out = model(
                    eeg, c_ids, c_lens, wp_ids, wp_lens, class_ids, class_lens,
                    cons_targets, ngram_targets, boundary_targets,
                    text_embs=text_embs, neg_text_bank=neg_text_bank
                )
                loss = (
                    8.0 * out["loss_wp_ctc"] +
        ...
        print(
            f"Epoch {epoch:3d} | WCTC:{tot_wp/n:.3f} CCTC:{tot_char/n:.3f} ACTC:{tot_class/n:.3f} "
            f"CONS:{tot_cons/n:.3f} SEM:{tot_sem/n:.3f} NGRAM:{tot_ngram/n:.3f} "
        ...
        best_match_top1, best_oracle_wer = _validate_phase1(
            model, val_loader, epoch, ckpt_prefix, best_match_top1, best_oracle_wer
        )

    return best_train_ctc, best_match_top1, best_oracle_wer


def run_phase1_curriculum(model, epochs_p1, train_loader_mix, train_loader_pure,
                          val_loader, ckpt_prefix, start_epoch=0,
                          best_train_ctc=float("inf"), best_match_top1=-1.0,
    ...
    if current_epoch < mix_epochs and train_loader_mix is not None:
        ...
        print(f"[Phase1] Stage A: frozen semantic alignment for {stage_epochs} epochs")
        best_train_ctc, best_match_top1, best_oracle_wer = _train_phase1_stage(
            ...
            best_match_top1=best_match_top1, best_oracle_wer=best_oracle_wer,
            freeze_encoder=True,
            stage_name="frozen warm-start + semantic auxiliary")
        current_epoch = mix_epochs

    if current_epoch < epochs_p1:
        stage_epochs = epochs_p1 - current_epoch
        print(f"[Phase1] Stage B: low-LR semantic separation for {stage_epochs} epochs")
        best_train_ctc, best_match_top1, best_oracle_wer = _train_phase1_stage(
            ...
            stage_name="last-4-layer low-LR semantic adaptation")
...
@app.function(image=image, gpu="H100", timeout=86400,
              volumes={"/data": data_vol, "/persist": ckpt_vol, "/v17": v17_vol, "/v14": v14_vol, "/v13": v13_vol, "/v20": v20_vol, "/v50": v50_vol},
              retries=modal.Retries(max_retries=5, backoff_coefficient=1.0, initial_delay=10.0))
def run_pipeline(epochs_p1: int = 6):
    ...
    global TEXT_EMB_LOOKUP, RETRIEVAL_TEXTS, RETRIEVAL_TEXT_TO_ID, RETRIEVAL_EMBS
    ...
    all_texts_for_teacher = [sample["text"] for sample in ds_zuco.samples]
    if len(ds_inner) > 0:
        all_texts_for_teacher.extend(sample["text"] for sample in ds_inner.samples)
    NGRAM_LIST, NGRAM_TO_ID = build_ngram_vocab(train_texts_for_vocab, max_features=256)
    SP_MODEL = build_sentencepiece_model(train_texts_for_vocab, model_prefix="/tmp/v20x_wordpiece", vocab_size=192)
    WP_VOCAB_SIZE = SP_MODEL.get_piece_size() + 1
    TEXT_EMB_LOOKUP, _, _ = build_text_embedding_lookup(all_texts_for_teacher)
    RETRIEVAL_TEXTS = sorted({
        canonicalize_text(text) for text in train_texts_zuco if canonicalize_text(text)
    })
    RETRIEVAL_TEXT_TO_ID = {text: i for i, text in enumerate(RETRIEVAL_TEXTS)}
    if RETRIEVAL_TEXTS:
        RETRIEVAL_EMBS = torch.stack([
            torch.from_numpy(TEXT_EMB_LOOKUP[text]).float() for text in RETRIEVAL_TEXTS
        ], dim=0)
    else:
        RETRIEVAL_EMBS = torch.empty(0, SEMANTIC_DIM, dtype=torch.float32)
    print(f"[V20x] ngram vocab size: {len(NGRAM_LIST)}")
    print(f"[V20x] wordpiece vocab size: {WP_VOCAB_SIZE - 1}")
    print(f"[V20x] retrieval bank size: {len(RETRIEVAL_TEXTS)}")

    model = EEG_CTC_V20X(ngram_dim=len(NGRAM_LIST), wp_vocab_size=WP_VOCAB_SIZE).to(torch.bfloat16).cuda()
    model.char_lm = CharNGramLM(train_texts_for_vocab, order=3)

    # ── Resume logic ──────────────────────────────────────────────────────────
    import glob as _glob
    p1_ckpts = sorted(_glob.glob("/persist/v20x_phase1_ep*.pt"))
    best_p1 = "/persist/v20x_phase1_best.pt"
    final = "/persist/v20x_final.pt"
    p1_start_epoch = 0
    best_match_top1 = -1.0
    best_oracle_wer = float("inf")
    ...
    print(f"\n{'='*60}")
    print(" V20x: semantic sentence alignment + open-vocab decode")
    print(" V17 warm-start + wordpiece/char decode + semantic teacher loss")
    print(f" {'(RESUMED)' if p1_ckpts else 'FRESH START'}")
    print(f"{'='*60}\n")

    best_train_ctc, best_match_top1, best_oracle_wer = run_phase1_curriculum(
        model, epochs_p1=epochs_p1,
        train_loader_mix=train_loader_mix,
        train_loader_pure=train_loader_pure,
        val_loader=val_loader,
        ckpt_prefix="/persist/v20x",
        start_epoch=p1_start_epoch,
        best_train_ctc=best_train_ctc,
        best_match_top1=best_match_top1,
    ...
    torch.save(model.state_dict(), "/persist/v20x_final.pt")
    ckpt_vol.commit()
    print("\n✓ V20x complete. Saved /persist/v20x_final.pt")


@app.local_entrypoint()
def main(mode: str = "pipeline", epochs_p1: int = 6):
    if mode == "pipeline":
        print(f"Launching V20x: Phase1={epochs_p1}ep")
