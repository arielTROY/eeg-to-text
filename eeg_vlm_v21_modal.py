...
app = modal.App("eeg-vlm-v21-phonology")

ckpt_vol = modal.Volume.from_name("bt-checkpoints-v21", create_if_missing=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
# V21: phonology-aligned open-text decoding.
#
# What failed recently:
#   [1] semantic teacher loss improved oracle beam quality but retrieval stayed
#       completely unaligned, so the supervision level was wrong
#   [2] decoder-only changes could not fix ranking when the EEG embedding lacked
#       phonological structure
#   [3] longer unfrozen schedules kept erasing consonant detail
#
# What worked:
#   [1] V17 had the best early beam quality
#   [2] the EEG-as-video path is recovering local chunks and some consonants
#
# This branch keeps the open-vocab V17 decode path and changes:
#   [1] sentence semantics -> phonological targets built from pronunciation
#   [2] phonological contrastive loss with hard negatives from a memory bank
#   [3] phonology-aware beam reranking
#   [4] frozen encoder by default so the decoder heads cannot destroy the early
#       VideoMAE / V17 alignment regime
# The goal is to improve both beam contents and beam ranking while staying
# compatible with later open-text decoding.
# ═══════════════════════════════════════════════════════════════════════════════
...
class EEG_CTC_V21(nn.Module):
    def __init__(self, ngram_dim, wp_vocab_size, phono_dim):
        ...
        self.phono_loss_fn = nn.BCEWithLogitsLoss()
    ...
    def forward(self, traj, char_ids, char_lens, wp_ids, wp_lens, class_ids, class_lens,
                cons_targets, ngram_targets, boundary_targets, phono_targets=None,
                neg_phono_bank=None):
        B = traj.shape[0]
        device = traj.device
        ...
        phono_logits = self.phono_head(pooled)
        ...
        if phono_targets is not None:
            loss_phono_bce = self.phono_loss_fn(phono_logits, phono_targets)
            eeg_phono = F.normalize(phono_logits, dim=-1)
            target_phono = F.normalize(phono_targets, dim=-1)
            pos_sim = torch.sum(eeg_phono * target_phono, dim=-1)
            if neg_phono_bank is not None and neg_phono_bank.shape[0] > 0:
                bank = neg_phono_bank.to(device=device, dtype=eeg_phono.dtype)
                all_targets = torch.cat([target_phono, bank], dim=0)
                sims = eeg_phono @ all_targets.T / 0.07
                targets = torch.arange(B, device=device)
                loss_phono_contrast = F.cross_entropy(sims, targets)
                neg_sims = sims[:, B:] if bank.shape[0] > 0 else sims.new_empty((B, 0))
            else:
                sims = eeg_phono @ target_phono.T / 0.07
                targets = torch.arange(B, device=device)
                loss_phono_contrast = (F.cross_entropy(sims, targets) + F.cross_entropy(sims.T, targets)) / 2.0
                neg_mask = ~torch.eye(B, dtype=torch.bool, device=device)
                neg_sims = sims.masked_select(neg_mask).view(B, B - 1) if B > 1 else sims.new_empty((B, 0))
            if neg_sims.numel() > 0:
                hardest_neg = neg_sims.max(dim=1).values
                loss_phono_margin = F.relu(0.20 - pos_sim + hardest_neg).mean()
            else:
                loss_phono_margin = torch.tensor(0.0, device=device)
            loss_phono = loss_phono_bce + loss_phono_contrast + 0.5 * loss_phono_margin
        else:
    ...
    def decode_ctc(self, traj, beam_width=16, nbest=5):
        ...
        eeg_phono = F.normalize(self.phono_head(v_seq.mean(dim=1)), dim=-1).float().cpu().numpy()
        ...
        merged = []
        for char_beams, wp_beams, ngp, cp, bp, ph in zip(char_groups, wp_groups, ngram_probs, cons_probs, boundary_probs, eeg_phono):
            cands = merge_char_wp_candidates(char_beams, wp_beams, ngp, cp, bp, nbest=nbest)
            merged.append(rerank_with_phonology(cands, ph, alpha=0.45))
        return merged
    ...
    def phonological_scores(self, traj):
        ...
        eeg_phono = F.normalize(self.phono_head(pooled), dim=-1)
        if PHONO_RETRIEVAL_EMBS is None or PHONO_RETRIEVAL_EMBS.numel() == 0:
            return torch.empty((traj.shape[0], 0), device=traj.device)
        bank = PHONO_RETRIEVAL_EMBS.to(device=traj.device, dtype=eeg_phono.dtype)
        return eeg_phono @ bank.T
...
def set_phase1_trainable(model, freeze_encoder):
    ...
    for p in model.cons_head.parameters():
        p.requires_grad = True
    for p in model.phono_head.parameters():
        p.requires_grad = True
...
def make_phase1_optimizer(model, steps_per_epoch, epochs, freeze_encoder):
    ...
    add_group(model.phono_head.parameters(), 1.2e-4 if freeze_encoder else 1.5e-4, "phono")
...
def _train_phase1_stage(model, epochs, train_loader, val_loader, ckpt_prefix,
    ...
    for local_epoch in range(1, epochs + 1):
        ...
        model.train()
        tot_char = tot_wp = tot_class = tot_cons = tot_phono = tot_ngram = tot_bnd = tot_div = 0.0
        optimizer.zero_grad()

        for step, batch in enumerate(train_loader):
            eeg, _, c_ids, c_lens, wp_ids, wp_lens, class_ids, class_lens, cons_targets, ngram_targets, boundary_targets, phono_targets = batch
...
def run_phase1_curriculum(model, epochs_p1, train_loader_mix, train_loader_pure,
    ...
    if current_epoch < mix_epochs and train_loader_mix is not None:
        ...
        print(f"[Phase1] Stage A: frozen phonological alignment for {stage_epochs} epochs")
        best_train_ctc, best_phono_top1, best_oracle_wer = _train_phase1_stage(
            ...
            freeze_encoder=True,
            stage_name="frozen warm-start + phonological auxiliary")
        current_epoch = mix_epochs

    if current_epoch < epochs_p1:
        stage_epochs = epochs_p1 - current_epoch
        print(f"[Phase1] Stage B: low-LR phonological adaptation for {stage_epochs} epochs")
        best_train_ctc, best_phono_top1, best_oracle_wer = _train_phase1_stage(
            ...
            stage_name="last-4-layer low-LR phonological adaptation")
...
def run_pipeline(epochs_p1: int = 3):
    ...
    global PHONE_BIGRAMS, PHONE_BIGRAM_TO_ID, PHONO_VECTOR_LOOKUP
    global PHONO_RETRIEVAL_TEXTS, PHONO_RETRIEVAL_TEXT_TO_ID, PHONO_RETRIEVAL_EMBS
    ...
    PHONE_BIGRAMS, PHONE_BIGRAM_TO_ID = build_phone_bigram_vocab(all_texts_for_teacher, max_features=384)
    PHONO_VECTOR_LOOKUP = build_phono_lookup(all_texts_for_teacher)
    SP_MODEL = build_sentencepiece_model(train_texts_for_vocab, model_prefix="/tmp/v21_wordpiece", vocab_size=192)
    WP_VOCAB_SIZE = SP_MODEL.get_piece_size() + 1
    PHONO_RETRIEVAL_TEXTS = sorted({
        canonicalize_text(text) for text in train_texts_zuco if canonicalize_text(text)
    })
    PHONO_RETRIEVAL_TEXT_TO_ID = {text: i for i, text in enumerate(PHONO_RETRIEVAL_TEXTS)}
    if PHONO_RETRIEVAL_TEXTS:
        PHONO_RETRIEVAL_EMBS = torch.stack([
            torch.from_numpy(phono_vector_for_text(text)).float() for text in PHONO_RETRIEVAL_TEXTS
        ], dim=0)
    else:
        PHONO_RETRIEVAL_EMBS = torch.empty(0, phono_vector_dim(), dtype=torch.float32)
    print(f"[V21] ngram vocab size: {len(NGRAM_LIST)}")
    print(f"[V21] phone bigram vocab size: {len(PHONE_BIGRAMS)}")
    print(f"[V21] wordpiece vocab size: {WP_VOCAB_SIZE - 1}")
    print(f"[V21] phonology bank size: {len(PHONO_RETRIEVAL_TEXTS)}")
    ...
    p1_ckpts = sorted(_glob.glob("/persist/v21_phase1_ep*.pt"))
    best_p1 = "/persist/v21_phase1_best.pt"
    final = "/persist/v21_final.pt"
    p1_start_epoch = 0
    best_phono_top1 = -1.0
    best_oracle_wer = float("inf")
    ...
    print(f"\n{'='*60}")
    print(" V21: phonology-aligned open-vocab decode")
    print(" V17 warm-start + wordpiece/char decode + phonological contrastive loss")
    ...
    torch.save(model.state_dict(), "/persist/v21_final.pt")
    ckpt_vol.commit()
    print("\n✓ V21 complete. Saved /persist/v21_final.pt")


@app.local_entrypoint()
def main(mode: str = "pipeline", epochs_p1: int = 3):
    if mode == "pipeline":
        print(f"Launching V21: Phase1={epochs_p1}ep")
