...
app = modal.App("eeg-vlm-v9-translator")

ckpt_vol = modal.Volume.from_name("bt-checkpoints-v9", create_if_missing=True)
...
class MultiScaleRasterizer(nn.Module):
    ...
    def forward(self, x):
        # x is (B, T, Ch) where Ch = 64 * 4 bands = 256
        B, T, Ch = x.shape
        # x_split: (B*T, 4, 64)
        x_split = x.reshape(B*T, 4, 64)
        # Apply projection to each channel: (B*T, 4, 4096)
        proj = x_split @ self.W.T.to(device=x.device, dtype=x.dtype)
        # Reshape to (B, T, 4, 64, 64)
        imgs = proj.reshape(B, T, 4, 64, 64)
        return imgs

class CrossAttentionBridge(nn.Module):
    def __init__(self, in_ch=768, out_ch=1024, n_latents=64):
        super().__init__()
        # V9: Perceiver-style Cross-Attention Bridge
    ...
    def forward(self, x):
        # x is (B, frames, 768)
        B = x.shape[0]
        q = self.latents.unsqueeze(0).repeat(B, 1, 1) # (B, 64, 768)
        # Cross-attend: Queries = Latents, Keys/Values = Brain sequence
        x, _ = self.mha(self.ln_q(q), self.ln_k(x), x)
        return self.proj(x)

class EEG_VLM_V9(nn.Module):
    def __init__(self, q_name):
        ...
        v_cfg = VideoMAEConfig(
            num_channels=4, # V9: Volt + Alpha + Beta + Gamma
        ...
        self.bridge = CrossAttentionBridge(768, 1024)
    ...
    def forward(self, traj, char_ids, char_lens, input_ids=None, labels=None):
        ...
        if input_ids is not None:
            ...
            # V9: Diversity Loss (Structural sanity)
            probs = F.softmax(ctc_logits, dim=-1) # (T, B, Vocab)
            ...
            return out.loss, loss_ctc, loss_lock, loss_div
        return None, loss_ctc, loss_lock, torch.tensor(0.0, device=traj.device)
...
def normalize_eeg(eeg, target_ch=64, target_t=128):
    ...
    # V9: Full-Spectrum Spectral Injection (Volt, Alpha, Beta, Gamma)
    from scipy.signal import butter, filtfilt
    def get_band_env(b_low, b_high, data):
        try:
            nyq = 64.0 # Approx for 128Hz
            b, a = butter(4, [b_low/nyq, b_high/nyq], btype='bandpass')
            f = filtfilt(b, a, data, axis=1)
            env = np.abs(f)
            # V8.1: Temporal smoothing for envelope (5-frame window)
            return np.convolve(env.flatten(), np.ones(5)/5, mode='same').reshape(env.shape)
        except: return np.zeros_like(data)

    alpha = get_band_env(8, 13, eeg)
    beta  = get_band_env(13, 30, eeg)
    gamma = get_band_env(30, 100, eeg)

    # Output shape (target_t, target_ch * 4)
    # Voltage first 64, then Alpha, Beta, Gamma
    combined = np.concatenate([eeg.T, alpha.T, beta.T, gamma.T], axis=1) # (128, 256)
...
def train_v9(epochs=30, phase=1):
    ...
    model = EEG_VLM_V9(q_name).to(torch.bfloat16).cuda()
    
    if phase == 2:
        print("--- PHASE 2: Word-Exact Locking (V9-CA) ---")
        # Load Phase 1 weights (Epoch 15)
        ckpt_path = Path("/persist/v9_phase1_epoch15.pt")
        if ckpt_path.exists():
    ...
    for epoch in range(1, epochs + 1):
        model.train()
        total_lm, total_ctc, total_lock, total_div = 0, 0, 0, 0
        for eeg, input_ids, labels, c_ids, c_lens in train_loader:
            ...
            with torch.amp.autocast('cuda', dtype=torch.bfloat16):
                loss_lm, loss_ctc, loss_lock, loss_div = model(eeg, c_ids, c_lens, input_ids, labels)
                
                # V9.1: ANNEALED LOCKING
                if phase == 2:
                    current_penalty = 1.0 + (14.0 * min(epoch/10, 1.0))
                else:
                    current_penalty = 0.5
                
                # V9 Loss Equation: LM + CTC + Lock + Diversity
                loss = loss_lm + 1.0 * loss_ctc + current_penalty * loss_lock + 2.0 * loss_div
            
            loss.backward()
            optimizer.step()
            total_lm += loss_lm.item(); total_ctc += loss_ctc.item(); total_lock += loss_lock.item(); total_div += loss_div.item()
            
        print(f"Epoch {epoch} | LM: {total_lm/len(train_loader):.3f} | CTC: {total_ctc/len(train_loader):.3f} | Lock: {total_lock/len(train_loader):.4f} | Div: {total_div/len(train_loader):.4f}")
        ...
        if epoch % 3 == 0:
            ...
            torch.save(model.state_dict(), f"/persist/v9_phase{phase}_epoch{epoch}.pt")

@app.local_entrypoint()
def main(phase: int = 1, epochs: int = 40):
    train_v9.remote(epochs=epochs, phase=phase)
