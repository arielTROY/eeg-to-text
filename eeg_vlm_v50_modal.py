...
app = modal.App("eeg-vlm-v50")

ckpt_vol = modal.Volume.from_name("bt-checkpoints-v50", create_if_missing=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
# V5.0: Keep the V20/V4.2 path that works, then push past the consonant ceiling.
#
# What V4.2 proved:
...
#
# Big upgrade:
#   [M1] Add motion channels (temporal derivatives of raw EEG + gamma envelope)
#        so VideoMAE sees fast transients, not only smooth band topographies.
#   [M2] Expand the old 4→3 channel adapter into a 6→3 adapter while preserving
#        the proven V20 weights in the first four inputs.
#   [M3] Use a Phase 1 lexical curriculum: early mixed ZuCo+InnerSpeech for
#        alignment, then pure ZuCo sentence consolidation.
#   [M4] Use beam-decoded CTC hints in Phase 2, keeping the stronger V41 path.
#
...
def run_pipeline(epochs_p1: int = 24, epochs_p2: int = 40):
    ...
    model = EEG_VLM_V50(q_name).to(torch.bfloat16).cuda()

    # ── Resume logic ──────────────────────────────────────────────────────────
    import glob as _glob
    p1_ckpts  = sorted(_glob.glob("/persist/v50_phase1_ep*.pt"))
    p2_ckpts  = sorted(_glob.glob("/persist/v50_phase2_ep*.pt"))
    best_p1   = "/persist/v50_phase1_best.pt"
    final     = "/persist/v50_final.pt"
    p1_start_epoch = 0

    if Path(final).exists():
        print("✓ V5.0 already complete.")
        return
    ...
    print(f"\n{'='*60}")
    print(f" V5.0: motion channels + lexical curriculum + beam-guided Phase 2")
    ...
    if not skip_p2:
        ...
        _train_phase(model, phase=2, epochs=epochs_p2_remaining,
                     ...
                     tokenizer=tokenizer, ckpt_prefix="/persist/v50",
                     mem_bank=mem_bank)

    torch.save(model.state_dict(), "/persist/v50_final.pt")
    ckpt_vol.commit()
    print("\n✓ V5.0 complete. Saved /persist/v50_final.pt")


@app.local_entrypoint()
def main(mode: str = "pipeline", epochs_p1: int = 24, epochs_p2: int = 40):
    if mode == "pipeline":
        print(f"Launching V5.0: Phase1={epochs_p1}ep, Phase2={epochs_p2}ep")
