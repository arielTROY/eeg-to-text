...
app = modal.App("eeg-vlm-v18-ctc")

ckpt_vol = modal.Volume.from_name("bt-checkpoints-v18", create_if_missing=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
# V18a: lexical memory transducer on top of the EEG-video encoder.
#
...
#   [1] a lexical-memory CTC head over phrases + words + char fallback
#   [2] auxiliary char CTC head
#   [3] boundary head over the encoder timeline
#   [4] joint decode from char beams and lexical-memory beams
# The intent is to predict real lexical units directly instead of spelling from
...
def tokenize_text_units(text):
    import re
    return re.findall(r"[a-z0-9']+|[.,!?()\"]| +", normalize_text(text))


def build_lexical_memory(texts, max_phrases=320, max_words=640):
    from collections import Counter

    word_counter = Counter()
    phrase_counter = Counter()
    for text in texts:
        tokens = tokenize_text_units(text)
        words = [tok for tok in tokens if tok and tok[0].isalnum()]
        for word in words:
            if len(word) >= 3:
                word_counter[word] += 1
        for n in (3, 2):
    ...
    phrases = [p for p, _ in phrase_counter.most_common(max_phrases)]
    words = [w for w, _ in word_counter.most_common(max_words)]
    chars = [c for c in CHAR_VOCAB if c != "_"]
    units = phrases + [w for w in words if w not in set(phrases)] + chars
    return units, {u: i + 1 for i, u in enumerate(units)}, set(phrases), set(words)


def text_to_lex_ids(text):
    tokens = tokenize_text_units(text)
    lex_ids = []
    i = 0
    while i < len(tokens):
        ...
        if tok and tok[0].isalnum():
            ...
            for span in (5, 3):
                if i + span <= len(tokens):
                    ...
                    if span == 5 and chunk[1] == " " and chunk[3] == " ":
                        phrase = f"{chunk[0]} {chunk[2]} {chunk[4]}"
                        if phrase in PHRASE_VOCAB:
                            matched = phrase
                            matched_len = 5
                            break
                    if span == 3 and chunk[1] == " ":
                        phrase = f"{chunk[0]} {chunk[2]}"
                        if phrase in PHRASE_VOCAB:
                            matched = phrase
                            matched_len = 3
                            break
            if matched is not None:
                lex_ids.append(LEX_UNIT_TO_ID[matched])
                i += matched_len
                continue
            if tok in WORD_VOCAB and tok in LEX_UNIT_TO_ID:
                lex_ids.append(LEX_UNIT_TO_ID[tok])
            else:
                for ch in tok:
                    if ch in LEX_UNIT_TO_ID:
                        lex_ids.append(LEX_UNIT_TO_ID[ch])
            i += 1
        else:
            for ch in tok:
                if ch in LEX_UNIT_TO_ID:
...
def decode_lex_ids(ids):
    parts = []
    for idx in ids:
        idx = int(idx)
        if idx <= 0 or idx > len(LEX_UNIT_LIST):
            continue
        parts.append(LEX_UNIT_LIST[idx - 1])
...
def text_to_boundary_target(text, seq_len=256):
    ...
    lex_ids = text_to_lex_ids(text)
    if len(lex_ids) <= 1:
        return target
    boundary_positions = np.linspace(0, seq_len - 1, len(lex_ids) + 1, dtype=int)[1:-1]
...
def beam_ctc_decode_lex_nbest(logits, beam_width=12, nbest=5, blank_penalty=0.15, diversity_weight=0.12):
    log_probs = F.log_softmax(logits, dim=-1)
    T, B, V = log_probs.shape
    results = []
    for b in range(B):
        ...
        for t in range(T):
            new_beams = {}
            frame_lp = log_probs[t, b].clone()
            frame_lp[LEX_BLANK_ID] -= blank_penalty
            topk_vals, topk_ids = frame_lp.topk(min(beam_width * 3, V))
            for seq, last_tok, ctc_score in beams:
                for val, tok_id in zip(topk_vals.tolist(), topk_ids.tolist()):
                    new_ctc_score = ctc_score + val
                    if tok_id == LEX_BLANK_ID or tok_id == last_tok:
                        key = tuple(seq)
                        existing = new_beams.get(key)
            ...
            while remaining and len(selected) < max(beam_width, nbest):
                ...
                for idx, cand in enumerate(remaining):
                    seq, _, score = cand
                    text = decode_lex_ids(seq)
                    if selected:
                        overlap = max(_common_prefix_ratio(text, decode_lex_ids(s[0])) for s in selected)
                    else:
                        overlap = 0.0
...
def merge_char_lex_candidates(char_beams, lex_beams, ngram_probs, cons_probs, boundary_probs, nbest=5):
    ...
    for text, score, tok_len in lex_beams:
        norm = normalize_text(text).strip()
        if not norm:
            continue
        vowel_match = max(0.0, 1.0 - min(compute_cer(_vowel_skeleton(norm), char_vowels), 1.5))
...
class EEG_CTC_V18(nn.Module):
    def __init__(self, ngram_dim, lex_vocab_size, lex_dim=256):
        super().__init__()
        ...
        self.lex_query = nn.Sequential(
            nn.LayerNorm(768),
            nn.Linear(768, lex_dim),
        )
        self.lex_memory = nn.Parameter(torch.randn(lex_vocab_size, lex_dim) * 0.02)
        ...
        self.lex_ctc_loss_fn = nn.CTCLoss(blank=LEX_BLANK_ID, zero_infinity=True)
    ...
    def forward(self, traj, char_ids, char_lens, lex_ids, lex_lens, class_ids, class_lens,
        ...
        lex_q = F.normalize(self.lex_query(v_seq), dim=-1)
        lex_mem = F.normalize(self.lex_memory, dim=-1)
        lex_logits = torch.matmul(lex_q, lex_mem.T).transpose(0, 1) * 10.0
        ...
        loss_lex_ctc = self.lex_ctc_loss_fn(
            F.log_softmax(lex_logits, dim=-1), lex_ids, input_lens, lex_lens)
        ...
        probs = F.softmax(lex_logits, dim=-1)
    ...
    def decode_ctc(self, traj, beam_width=16, nbest=5):
        ...
        lex_groups = beam_ctc_decode_lex_nbest(
            lex_logits, beam_width=beam_width, nbest=max(nbest, 5))
        merged = []
        for char_beams, lex_beams, ngp, cp, bp in zip(char_groups, lex_groups, ngram_probs, cons_probs, boundary_probs):
            merged.append(merge_char_lex_candidates(char_beams, lex_beams, ngp, cp, bp, nbest=nbest))
        return merged
...
class EEGDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        lex_ids = text_to_lex_ids(s['text'])
        ...
        return eeg, s['text'], char_ids, lex_ids, class_ids, cons_target, ngram_target, boundary_target


def collate_fn(batch):
    eegs, texts, char_ids_list, lex_ids_list, class_ids_list, cons_targets, ngram_targets, boundary_targets = zip(*batch)
    ...
    lex_lens = torch.tensor([len(w) for w in lex_ids_list], dtype=torch.long)
    max_lex = max(lex_lens) if max(lex_lens) > 0 else 1
    lex_pad = torch.zeros(len(lex_ids_list), max_lex, dtype=torch.long)
    for i, w in enumerate(lex_ids_list):
        if len(w) > 0:
            lex_pad[i, :len(w)] = torch.tensor(w)
    ...
    return eegs, list(texts), char_pad, char_lens, lex_pad, lex_lens, class_pad, class_lens, cons_tensor, ngram_tensor, boundary_tensor
...
def set_videomae_last_layers_trainable(video_enc, n_last=4):
    ...
    if hasattr(video_enc, "layernorm"):
...
def set_phase1_trainable(model, freeze_encoder):
    ...
    for p in model.ctc_head.parameters():
        p.requires_grad = True
    for p in model.lex_query.parameters():
        p.requires_grad = True
    model.lex_memory.requires_grad_(True)
...
def make_phase1_optimizer(model, steps_per_epoch, epochs, freeze_encoder):
    ...
    add_group(model.lex_query.parameters(), 1.2e-4 if freeze_encoder else 1.4e-4, "lex_q")
    add_group([model.lex_memory], 1.2e-4 if freeze_encoder else 1.4e-4, "lex_mem")
...
def _train_phase1_stage(model, epochs, train_loader, val_loader, ckpt_prefix,
    ...
    for local_epoch in range(1, epochs + 1):
        ...
        model.train()
        tot_char = tot_lex = tot_class = tot_cons = tot_ngram = tot_bnd = tot_div = 0.0
        optimizer.zero_grad()

        for step, batch in enumerate(train_loader):
            eeg, _, c_ids, c_lens, lex_ids, lex_lens, class_ids, class_lens, cons_targets, ngram_targets, boundary_targets = batch
            eeg = eeg.to(device).to(torch.bfloat16)
            c_ids = c_ids.to(device)
            c_lens = c_lens.to(device)
            lex_ids = lex_ids.to(device)
            lex_lens = lex_lens.to(device)
            ...
            with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                out = model(
                    eeg, c_ids, c_lens, lex_ids, lex_lens, class_ids, class_lens,
                    cons_targets, ngram_targets, boundary_targets
                )
                loss = (
                    10.0 * out["loss_lex_ctc"] +
        ...
        print(
            f"Epoch {epoch:3d} | LCTC:{tot_lex/n:.3f} CCTC:{tot_char/n:.3f} ACTC:{tot_class/n:.3f} "
            f"CONS:{tot_cons/n:.3f} NGRAM:{tot_ngram/n:.3f} BND:{tot_bnd/n:.3f} Div:{tot_div/n:.4f} | LR0:{lr:.2e}"
        )

        if (tot_lex / n) < best_train_ctc:
            best_train_ctc = tot_lex / n
            print(f"  ✓ New best train LCTC {best_train_ctc:.3f}")
...
def run_phase1_curriculum(model, epochs_p1, train_loader_mix, train_loader_pure,
    ...
    if current_epoch < epochs_p1:
        ...
        print(f"[Phase1] Single stage: frozen lexical-memory fit for {stage_epochs} epochs")
        best_train_ctc, best_val_cer = _train_phase1_stage(
            ...
            stage_name="frozen lexical-memory transducer")
    return best_train_ctc, best_val_cer
...
def run_pipeline(epochs_p1: int = 3):
    ...
    global NGRAM_LIST, NGRAM_TO_ID, LEX_UNIT_LIST, LEX_UNIT_TO_ID, PHRASE_VOCAB, WORD_VOCAB
    ...
    LEX_UNIT_LIST, LEX_UNIT_TO_ID, PHRASE_VOCAB, WORD_VOCAB = build_lexical_memory(
        train_texts_for_vocab, max_phrases=320, max_words=640
    )
    print(f"[V18] ngram vocab size: {len(NGRAM_LIST)}")
    print(f"[V18] lexical memory size: {len(LEX_UNIT_LIST)}")
    print(f"[V18] phrase units: {len(PHRASE_VOCAB)}  word units: {len(WORD_VOCAB)}")

    model = EEG_CTC_V18(ngram_dim=len(NGRAM_LIST), lex_vocab_size=len(LEX_UNIT_LIST) + 1).to(torch.bfloat16).cuda()
    model.char_lm = CharNGramLM(train_texts_for_vocab, order=3)

    # ── Resume logic ──────────────────────────────────────────────────────────
    import glob as _glob
    p1_ckpts = sorted(_glob.glob("/persist/v18_phase1_ep*.pt"))
    best_p1 = "/persist/v18_phase1_best.pt"
    final = "/persist/v18_final.pt"
    p1_start_epoch = 0
    best_val_cer = float("inf")
    best_train_ctc = float("inf")

    if Path(final).exists():
        print("✓ V18 already complete.")
        return
    ...
    print(f"\n{'='*60}")
    print(" V18a: lexical-memory CTC main + char fallback")
    print(" V14 warm-start + phrase/word memory + short frozen fit")
    ...
    torch.save(model.state_dict(), "/persist/v18_final.pt")
    ckpt_vol.commit()
    print("\n✓ V18 complete. Saved /persist/v18_final.pt")


@app.local_entrypoint()
def main(mode: str = "pipeline", epochs_p1: int = 3):
    if mode == "pipeline":
        print(f"Launching V18: Phase1={epochs_p1}ep")
