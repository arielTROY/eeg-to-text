...
app      = modal.App("eeg-vlm-v800")
ckpt_vol = modal.Volume.from_name("bt-checkpoints-v800", create_if_missing=True)
...
class EEGV800(nn.Module):
    ...
    def forward(self, traj, sbert_embs, target_ids, target_mask,
        ...
        # V800: less LM weight, stronger alignment → prevents memorization collapse
        total = 0.5 * loss_lm + 1.5 * loss_align + 0.30 * loss_sent + 0.10 * loss_len
...
def load_warm_start(model):
    ...
    # Priority 1: V700 epoch-5 checkpoint (same T5 architecture)
    v700_dir = Path("/v700_ckpt")
    v700_ckpts = sorted(glob.glob(str(v700_dir / "v700_ep5*.pt")))
    if not v700_ckpts:
...
def train(model, train_ds, val_ds, tokenizer, device, epochs=40):
    ...
    for epoch in range(1, epochs + 1):
        ...
        if score > best_score or wer < best_wer:
            ...
            ckpt_path  = ckpt_dir / f"v800_ep{epoch}_div{div:.3f}_cr{cr:.3f}_wer{wer:.3f}.pt"
            torch.save({
...
def main():
    run_training.remote(epochs=20)  # V700 shows collapse starts ~ep10; early stop handles rest
