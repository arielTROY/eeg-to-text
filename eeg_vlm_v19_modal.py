...
app = modal.App("eeg-vlm-v19-rnnt")

ckpt_vol = modal.Volume.from_name("bt-checkpoints-v19", create_if_missing=True)
...
image = (
    ...
    .apt_install(["curl"])
    .pip_install([
        "torch>=2.4.0", "torchaudio>=2.4.0", "transformers>=4.45.0", "peft>=0.12",
...
# ═══════════════════════════════════════════════════════════════════════════════
# V19: open-vocabulary wordpiece transducer on top of the EEG-video encoder.
#
# What failed in V14-V16:
#   [1] character beams still carry mostly vowel skeletons and filler words
#   [2] reranking helps only when the right sentence is already in the beam
#   [3] oracle WER staying near 0.945 means the missing information is upstream
#
# What worked:
#   [1] V14 warm-start is the strongest stable base
#   [2] the video encoder is recovering local chunks, just not as the right unit
#
# This branch keeps the V14 encoder path intact and adds:
#   [1] a wordpiece RNN-T head as the main objective
#   [2] auxiliary char CTC
#   [3] boundary head
#   [4] joint decode from char beams and RNN-T wordpiece beams
# The intent is to replace frame-independent CTC spelling with an open-vocab
# transducer that conditions on previously emitted symbols.
# ═══════════════════════════════════════════════════════════════════════════════
...
def build_sentencepiece_model(texts, model_prefix="/tmp/v19_wordpiece", vocab_size=192):
...
def rnnt_beam_decode_wp_nbest(model, enc_seq, beam_width=4, nbest=5, max_symbols_per_timestep=2):
    results = []
    device = enc_seq.device
    B, T, _ = enc_seq.shape
    for b in range(B):
        enc = enc_seq[b]
        init_state = torch.zeros(1, 1, model.pred_dim, device=device, dtype=enc.dtype)
        start_token = torch.tensor([[WP_BLANK_ID]], device=device, dtype=torch.long)
        start_emb = model.pred_emb(start_token)
        pred0, init_state = model.pred_rnn(start_emb, init_state)
        pred0 = pred0[:, -1, :]
        beams = [(tuple(), 0.0, init_state, pred0)]

        for t in range(T):
            frame = enc[t:t + 1]
            expanded = []
            for seq, score, state, pred_vec in beams:
                blank_logits = model.join(frame, pred_vec)
                blank_logp = F.log_softmax(blank_logits, dim=-1)[0, WP_BLANK_ID].item()
                expanded.append((seq, score + blank_logp, state, pred_vec))

                emit_beams = [(seq, score, state, pred_vec)]
                for _ in range(max_symbols_per_timestep):
                    next_emit = []
                    for eseq, escore, estate, epred in emit_beams:
                        logits = model.join(frame, epred)
                        logp = F.log_softmax(logits, dim=-1)[0]
                        topk_vals, topk_ids = torch.topk(logp[1:], k=min(beam_width, logp.shape[0] - 1))
                        for val, tid in zip(topk_vals.tolist(), (topk_ids + 1).tolist()):
                            token = torch.tensor([[tid]], device=device, dtype=torch.long)
                            emb = model.pred_emb(token)
                            pred_out, new_state = model.pred_rnn(emb, estate)
                            new_pred = pred_out[:, -1, :]
                            new_seq = eseq + (tid,)
                            next_emit.append((new_seq, escore + val, new_state, new_pred))
                    if not next_emit:
                        break
                    next_emit = sorted(next_emit, key=lambda x: -x[1])[:beam_width]
                    expanded.extend(next_emit)
                    emit_beams = next_emit

            dedup = {}
            for seq, score, state, pred_vec in expanded:
                key = seq
                if key not in dedup or dedup[key][0] < score:
                    dedup[key] = (score, state, pred_vec)
            beams = sorted(
                [(seq, score, state, pred_vec) for seq, (score, state, pred_vec) in dedup.items()],
                key=lambda x: -x[1]
            )[:beam_width]

        decoded = []
        for seq, score, _, _ in beams[:nbest]:
            text = decode_wp_ids(seq)
            if text:
                decoded.append((text, score, len(seq)))
        if not decoded:
            decoded = [("", 0.0, 0)]
        score_tensor = torch.tensor([s for _, s, _ in decoded], dtype=torch.float32)
        probs = torch.softmax(score_tensor - score_tensor.max(), dim=0).tolist()
        results.append([(text, float(prob), tok_len) for (text, _, tok_len), prob in zip(decoded, probs)])
    return results
...
class EEG_RNNT_V19(nn.Module):
    def __init__(self, ngram_dim, wp_vocab_size, pred_dim=256, join_dim=384):
        super().__init__()
        ...
        self.pred_emb = nn.Embedding(wp_vocab_size, pred_dim)
        self.pred_rnn = nn.GRU(pred_dim, pred_dim, num_layers=1, batch_first=True)
        self.enc_proj = nn.Linear(768, join_dim)
        self.pred_proj = nn.Linear(pred_dim, join_dim)
        self.join_out = nn.Linear(join_dim, wp_vocab_size)
        ...
        self.rnnt_loss_fn = __import__("torchaudio").transforms.RNNTLoss(blank=WP_BLANK_ID, reduction="mean", fused_log_softmax=True)
    ...
    def join(self, enc_frame, pred_vec):
        enc_term = self.enc_proj(enc_frame)
        pred_term = self.pred_proj(pred_vec)
        return self.join_out(torch.tanh(enc_term + pred_term))
    ...
    def forward(self, traj, char_ids, char_lens, wp_ids, wp_lens, class_ids, class_lens,
        ...
        pred_in = F.pad(wp_ids, (1, 0), value=WP_BLANK_ID)
        pred_out, _ = self.pred_rnn(self.pred_emb(pred_in))
        enc_term = self.enc_proj(v_seq).unsqueeze(2)
        pred_term = self.pred_proj(pred_out).unsqueeze(1)
        rnnt_logits = self.join_out(torch.tanh(enc_term + pred_term))
        ...
        loss_rnnt = self.rnnt_loss_fn(rnnt_logits.float(), wp_ids.int(), input_lens.int(), wp_lens.int())
        ...
        probs = F.softmax(rnnt_logits.mean(dim=2), dim=-1)
    ...
    def decode_ctc(self, traj, beam_width=16, nbest=5):
        ...
        wp_groups = rnnt_beam_decode_wp_nbest(
            self, v_seq, beam_width=min(beam_width, 4), nbest=max(nbest, 5), max_symbols_per_timestep=2)
        merged = []
...
def collate_fn(batch):
    ...
    return eegs, list(texts), char_pad, char_lens, wp_pad, wp_lens, class_pad, class_lens, cons_tensor, ngram_tensor, boundary_tensor
...
def set_videomae_last_layers_trainable(video_enc, n_last=4):
    ...
    layernorm = getattr(video_enc, "layernorm", None)
...
def set_phase1_trainable(model, freeze_encoder):
    ...
    for p in model.ctc_head.parameters():
        p.requires_grad = True
    for p in model.pred_emb.parameters():
        p.requires_grad = True
    for p in model.pred_rnn.parameters():
        p.requires_grad = True
    for p in model.enc_proj.parameters():
        p.requires_grad = True
    for p in model.pred_proj.parameters():
        p.requires_grad = True
    for p in model.join_out.parameters():
        p.requires_grad = True
...
def make_phase1_optimizer(model, steps_per_epoch, epochs, freeze_encoder):
    ...
    add_group(model.pred_emb.parameters(), 1e-4 if freeze_encoder else 1.2e-4, "pred_emb")
    add_group(model.pred_rnn.parameters(), 1.1e-4 if freeze_encoder else 1.3e-4, "pred_rnn")
    add_group(model.enc_proj.parameters(), 1.0e-4 if freeze_encoder else 1.2e-4, "enc_proj")
    add_group(model.pred_proj.parameters(), 1.0e-4 if freeze_encoder else 1.2e-4, "pred_proj")
    add_group(model.join_out.parameters(), 1.0e-4 if freeze_encoder else 1.2e-4, "join")
...
def _train_phase1_stage(model, epochs, train_loader, val_loader, ckpt_prefix,
    ...
    for local_epoch in range(1, epochs + 1):
        ...
        model.train()
        tot_char = tot_rnnt = tot_class = tot_cons = tot_ngram = tot_bnd = tot_div = 0.0
        optimizer.zero_grad()

        for step, batch in enumerate(train_loader):
            eeg, _, c_ids, c_lens, wp_ids, wp_lens, class_ids, class_lens, cons_targets, ngram_targets, boundary_targets = batch
            ...
            with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                ...
                loss = (
                    ...
                    0.35 * out["loss_cons"] +
                    0.30 * out["loss_ngram"] +
                    0.75 * out["loss_boundary"] +
                    0.25 * out["loss_div"]
                ) / ACCUM_STEPS

            loss.backward()
            tot_char += out["loss_char_ctc"].item()
            tot_rnnt += out["loss_rnnt"].item()
        ...
        print(
            f"Epoch {epoch:3d} | RNNT:{tot_rnnt/n:.3f} CCTC:{tot_char/n:.3f} ACTC:{tot_class/n:.3f} "
            f"CONS:{tot_cons/n:.3f} NGRAM:{tot_ngram/n:.3f} BND:{tot_bnd/n:.3f} Div:{tot_div/n:.4f} | LR0:{lr:.2e}"
        )

        if (tot_rnnt / n) < best_train_ctc:
            best_train_ctc = tot_rnnt / n
            print(f"  ✓ New best train RNNT {best_train_ctc:.3f}")
...
def run_phase1_curriculum(model, epochs_p1, train_loader_mix, train_loader_pure,
    ...
    if current_epoch < epochs_p1:
        ...
        active_loader = train_loader_mix if train_loader_mix is not None else train_loader_pure
        print(f"[Phase1] Single stage: frozen RNNT fit for {stage_epochs} epochs")
        best_train_ctc, best_val_cer = _train_phase1_stage(
            model, epochs=stage_epochs, train_loader=active_loader,
            ...
            stage_name="frozen V14 warm-start + RNNT head")
    return best_train_ctc, best_val_cer
...
def run_pipeline(epochs_p1: int = 3):
    ...
    SP_MODEL = build_sentencepiece_model(train_texts_for_vocab, model_prefix="/tmp/v19_wordpiece", vocab_size=192)
    WP_VOCAB_SIZE = SP_MODEL.get_piece_size() + 1
    print(f"[V19] ngram vocab size: {len(NGRAM_LIST)}")
    print(f"[V19] wordpiece vocab size: {WP_VOCAB_SIZE - 1}")

    model = EEG_RNNT_V19(ngram_dim=len(NGRAM_LIST), wp_vocab_size=WP_VOCAB_SIZE).to(torch.bfloat16).cuda()
    model.char_lm = CharNGramLM(train_texts_for_vocab, order=3)

    # ── Resume logic ──────────────────────────────────────────────────────────
    import glob as _glob
    p1_ckpts = sorted(_glob.glob("/persist/v19_phase1_ep*.pt"))
    best_p1 = "/persist/v19_phase1_best.pt"
    final = "/persist/v19_final.pt"
    p1_start_epoch = 0
    best_val_cer = float("inf")
    best_train_ctc = float("inf")

    if Path(final).exists():
        print("✓ V19 already complete.")
        return
    ...
    print(f"\n{'='*60}")
    print(" V19: wordpiece RNNT main + char CTC auxiliary")
    print(" V14 warm-start + frozen 3-epoch transducer fit")
    ...
    torch.save(model.state_dict(), "/persist/v19_final.pt")
    ckpt_vol.commit()
    print("\n✓ V19 complete. Saved /persist/v19_final.pt")


@app.local_entrypoint()
def main(mode: str = "pipeline", epochs_p1: int = 3):
    if mode == "pipeline":
        print(f"Launching V19: Phase1={epochs_p1}ep")
