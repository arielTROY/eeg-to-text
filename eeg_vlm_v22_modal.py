...
app = modal.App("eeg-vlm-v22-topo-phone")

ckpt_vol = modal.Volume.from_name("bt-checkpoints-v22", create_if_missing=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
# V22: real scalp-topography + local phone supervision.
#
...
def build_sentencepiece_model(texts, model_prefix="/persist/v22_wordpiece", vocab_size=192):
...
class EEG_CTC_V22(nn.Module):
    ...
    def decode_ctc(self, traj, beam_width=16, nbest=5):
        ...
        merged = []
        for char_beams, wp_beams, ngp, cp, bp, ph, pp in zip(char_groups, wp_groups, ngram_probs, cons_probs, boundary_probs, eeg_phono, phone_probs):
            cands = merge_char_wp_candidates(char_beams, wp_beams, ngp, cp, bp, nbest=nbest)
            merged.append(rerank_with_phonology(cands, ph, alpha=0.35))
        return merged
...
def run_pipeline(epochs_p1: int = 3, use_innerspeech: bool = False):
    ...
    SP_MODEL = build_sentencepiece_model(train_texts_for_vocab, model_prefix="/persist/v22_wordpiece", vocab_size=192)
    ...
    print(f"[V22] ngram vocab size: {len(NGRAM_LIST)}")
    print(f"[V22] phone bigram vocab size: {len(PHONE_BIGRAMS)}")
    print(f"[V22] wordpiece vocab size: {WP_VOCAB_SIZE - 1}")
    print(f"[V22] phonology bank size: {len(PHONO_RETRIEVAL_TEXTS)}")
    ...
    p1_ckpts = sorted(_glob.glob("/persist/v22_phase1_ep*.pt"))
    best_p1 = "/persist/v22_phase1_best.pt"
    final = "/persist/v22_final.pt"
    p1_start_epoch = 0
    best_phono_top1 = -1.0
    best_oracle_wer = float("inf")
    ...
    print(f"\n{'='*60}")
    print(" V22: real topography + local phone supervision")
    print(" V17 warm-start + RGB topomaps + phone CTC + phonological rerank")
    ...
    torch.save(model.state_dict(), "/persist/v22_final.pt")
    ckpt_vol.commit()
    print("\n✓ V22 complete. Saved /persist/v22_final.pt")


@app.local_entrypoint()
def main(mode: str = "pipeline", epochs_p1: int = 3, use_innerspeech: bool = False):
    if mode == "pipeline":
        print(f"Launching V22: Phase1={epochs_p1}ep inner={use_innerspeech}")
