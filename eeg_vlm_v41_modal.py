...
app = modal.App("eeg-vlm-v41")

ckpt_vol = modal.Volume.from_name("bt-checkpoints-v41", create_if_missing=True)
v40_vol  = modal.Volume.from_name("bt-checkpoints-v40", create_if_missing=False)
...
# ═══════════════════════════════════════════════════════════════════════════════
# V4.1: Keep the useful EEG-MAE encoder from V4.0, drop phoneme CTC entirely.
#
# Root cause of V4.0 failure:
#   EEG-MAE pretraining helped the encoder, but the phoneme auxiliary CTC head
#   pulled Phase 1 away from usable character alignment. By epoch 9, CCTC was
#   still 3.129 and validation CTC was blank or just '.' with WER=1.000.
#
# Fix:
#   [M1] Reuse the best V4.0 EEG-MAE checkpoint as encoder initialization
#   [M2] Return to plain character CTC + diversity loss in Phase 1
#   [M3] Keep the V3.4 / V3.1b Phase 2 improvements unchanged
#
# All useful improvements kept:
#   [B1] Gamma band (30-100Hz)
#   [B2] Per-channel temporal smoothing
...
def load_v40_mae_encoder(model, ckpt_path="/v40/v40_mae_best.pt"):
    """Load only the EEG-MAE-adapted encoder weights from V4.0."""
    if not Path(ckpt_path).exists():
        print(f"V4.0 MAE checkpoint not found at {ckpt_path}")
        return False

    print(f"Loading V4.0 EEG-MAE encoder weights from {ckpt_path}...")
    src_sd = torch.load(ckpt_path, map_location="cpu")
    dst_sd = model.state_dict()
    prefixes = ("video_enc.", "ch_adapt.")
    n_copied = n_skipped = 0
    for k, v in src_sd.items():
    ...
    print(f"  ✓ Loaded {n_copied} EEG-MAE encoder tensors, skipped {n_skipped}")
    return n_copied > 0
...
def normalize_eeg(eeg, target_ch=64, target_t=1024):
    ...
    def band_env(lo, hi, data):
        try:
            ...
            # Per-channel smoothing (no cross-channel bleed)
            kernel   = np.ones(5) / 5
...
@app.function(image=image, gpu="H100", timeout=86400,
              volumes={"/data": data_vol, "/persist": ckpt_vol, "/v40": v40_vol},
...
def run_pipeline(epochs_p1: int = 40, epochs_p2: int = 40):
    ...
    ds_train = EEGDataset("/data/EEG_Text", augment='none')   # no augmentation in Phase 1 — CTC needs stable alignment
    ...
    model = EEG_VLM_V41(q_name).to(torch.bfloat16).cuda()

    # ── Resume logic ──────────────────────────────────────────────────────────
    import glob as _glob
    p1_ckpts  = sorted(_glob.glob("/persist/v41_phase1_ep*.pt"))
    p2_ckpts  = sorted(_glob.glob("/persist/v41_phase2_ep*.pt"))
    best_p1   = "/persist/v41_phase1_best.pt"
    final     = "/persist/v41_final.pt"

    if Path(final).exists():
        print("✓ V4.1 already complete.")
        return
    ...
    if p2_ckpts:
        ...
        if not load_v40_mae_encoder(model):
            load_pretrained_videomae_encoder(model.video_enc)

    print(f"\n{'='*60}")
    print(f" V4.1: V4.0 EEG-MAE encoder + plain char CTC + V3.4 Phase 2")
    ...
    if not skip_p1:
        _train_phase(model, phase=1, epochs=epochs_p1,
                     ...
                     tokenizer=tokenizer, ckpt_prefix="/persist/v41")
    ...
    torch.save(model.state_dict(), "/persist/v41_final.pt")
    ckpt_vol.commit()
    print("\n✓ V4.1 complete. Saved /persist/v41_final.pt")


@app.local_entrypoint()
def main(mode: str = "pipeline", epochs_p1: int = 40, epochs_p2: int = 40):
    if mode == "pipeline":
        print(f"Launching V4.1: Phase1={epochs_p1}ep, Phase2={epochs_p2}ep")
